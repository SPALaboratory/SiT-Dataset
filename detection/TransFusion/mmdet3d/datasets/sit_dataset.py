...
cls_label_map = {'car' : 'car', 'motorcycle': 'motorcycle', 'pedestrian': 'pedestrian', 'bicycle':'bicycle', 'bus':'bus'}

@DATASETS.register_module()
class SiT_Dataset(Custom3DDataset):
    r"""NuScenes Dataset.
    ...
    def get_data_info(self, index):
        ...
        if self.modality['use_camera']:
            ...
            for cam_type, cam_info in info['cams'].items():
                ...
                extrinsic = cam_info['cam_intrinsic'][1]
                #translation = cam_info['cam_intrinsic'][1][:3, 3].reshape(3, 1)
                projection_matrix = np.eye(4)

                projection_matrix[:3, :] = intrinsic@extrinsic
            ...
            for i, box in enumerate(boxes):
                ...
                if np.sqrt(box.velocity[0]**2 + box.velocity[1]**2) > 0.2:
                    if name in [
                        ...
                        attr = SiT_Dataset.DefaultAttribute[name]
                else:
    ...
    def evaluate(self,
        ...
        for i, t_ in enumerate(list(result_['results'].keys())):
            ...
            gt_bboxes_3d = data_['gt_boxes'] # 로컬좌표계(내가 이용해야 할 것), label_3d에 있는 좌표들은 글로벌 좌표계
            gt_labels_3d = data_['gt_names']
            # list_B = copy.deepcopy(list_A)
            gt_bboxes_3d_no_transform = copy.deepcopy(data_['gt_boxes']) # m 좌표계로 변환 X
            gt_labels_3d_no_transform = copy.deepcopy(data_['gt_names']) # m 좌표계로 변환 X
            # breakpoint()
            root_path = Path("./data/sit/")
            _save_path_ = "./range_gt_pred/range_40/"
            place = data_['token'].split('*')[0]
            scene = data_['token'].split('*')[1]
            frame = data_['token'].split('*')[2]
            velo_path = data_['lidar_path']
            odom_path_ = []
            ...
            mask = in_range_bev(gt_bboxes_3d, [-40., -40., 40., 40.]) 
           
             
            
            gt_bboxes_3d = gt_bboxes_3d_no_transform[mask]
            
            gt_labels_3d = gt_labels_3d_no_transform[mask.astype(np.bool)]
            
            data_['gt_boxes'] = gt_bboxes_3d
            data_['gt_names'] = gt_labels_3d

            #for visualization about gt & pred
            '''
            ...
            if vis_flag:
                bev_gt = nuscene_vis_gt(points, gt_bboxes_3d) # GT bbox의 BEV map
                # cv2.imwrite(save_path+"{}_{}_{}_gt.png".format(place, scene, frame), bev_gt)

                pred_boxes1 = []
                for j in range(len(result_['results'][data_['token']])):
                    pred_boxes1.append([])
                    # pred_boxes1[j].extend([result_['results'][data_['token']][j]['translation'],result_['results'][data_['token']][j]['size'],result_['results'][data_['token']][j]['rotation'][0]])
                    # pred_boxes1[j].extend(result_['results'][data_['token']][j]['translation'])
                    # pred_boxes1[j].extend(result_['results'][data_['token']][j]['size'])
                    # pred_boxes1[j].extend(result_['results'][data_['token']][j]['rotation'][0])
                    combined = result_['results'][data_['token']][j]['translation'] + result_['results'][data_['token']][j]['size'] + [result_['results'][data_['token']][j]['rotation'][0]]
                    pred_boxes1[j].append(combined)
                
                pred_boxes2 = []
                for kk in range(len(pred_boxes1)):
                    pred_boxes2.append(pred_boxes1[kk][0])
                pred_boxes3 = np.array(pred_boxes2)
                bev_pred = nuscene_vis_pred(points, pred_boxes3) # GT bbox의 BEV map
                # cv2.imwrite(save_path+"{}_{}_{}_gt.png".format(place, scene, frame), bev_gt)
                ...
                cv2.imwrite(_save_path_+"{}_{}_{}_intrgrated.png".format(place, scene, frame), bev_integrated)
                previous_scene = data_['token'].split('*')[1]
                # breakpoint()
            '''
            for ii in range(self.data_infos[i]['gt_boxes'].shape[0]):
                ...
                d_['size'] = np.abs(_boxes[3:6][[1,0,2]]).tolist() #for nuscene_eval : lwh(in model) -> wlh(nuscene_eval) # base
        ...
        # with open(_save_path_+"total_box_&_total_empty_scenes.txt", 'w', encoding='utf-8') as file:
        #     file.write(f"total gt boxes : {num_gt_boxes}\n")
        #     file.write(f"total empty scenes : {len(empty_scene)}\n")
        #     for item in empty_scene:
        #         file.write(f"{item}\n")
...
class SiT_NuScenesEval(DetectionSiTEval):
    def __init__(self,
                 nusc: NuScenes,
                 config: DetectionConfig,
                 result_path: str,
