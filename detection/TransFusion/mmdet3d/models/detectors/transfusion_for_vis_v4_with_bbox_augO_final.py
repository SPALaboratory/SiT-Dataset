    ...
    def forward_pts_train(self,
        ...
        def draw_center_near(image, qs):
            ...
            for point in qs:
                x, y = point
                # breakpoint()
                cv2.circle(image, (x+1,y+1), 5, (0,0,255),-1)
        ...
        def transform_box(gt_box, lidar2cam_rt):
            """
            LiDAR 좌표계에서 카메라 좌표계로 바운딩 박스를 변환합니다.
            
            Parameters:
            - gt_box: (x, y, z, l, w, h, rot)의 형태를 갖는 3D 바운딩 박스. 텐서 자료형.
            - lidar2cam_rt: LiDAR 좌표계를 카메라 좌표계로 변환하는 4x4 행렬. 텐서 자료형.
            
            Returns:
            - transformed_box: 카메라 좌표계로 변환된 3D 바운딩 박스.
            """
            x, y, z, l, w, h, rot = gt_box

            # 3D 바운딩 박스 중심 좌표 (homogeneous coordinate)
            center = torch.tensor([x, y, z, 1.0], dtype=torch.float32)
            ...
            # 중심 좌표를 원점으로 옮기고 회전 적용
            transformed_corners = []
            for corner in corners:
                transformed_corner = torch.matmul(rotation_matrix, corner[:3]) + center[:3]
                transformed_corner = torch.cat((transformed_corner, torch.tensor([1.0], dtype=torch.float32)))  # homogeneous coordinate
                transformed_corners.append(transformed_corner)

            # LiDAR to Camera 좌표 변환
            transformed_corners = torch.matmul(lidar2cam_rt, torch.stack(transformed_corners).T).T
        ...
        if vis_flag:
            ...
            # breakpoint()
            save_path = "./data/spa/gt_projection_aug_bbox/"
            data_path = "./data/spa/"
            for batch in range(len(bbox_list)):
                ...
                img_scale_factor = (
                    torch.tensor(img_metas[batch]['scale_factor'][:2]
                                        if 'scale_factor' in img_metas[batch].keys() else [1.0, 1.0]))
                                        
                img_flip = torch.tensor(img_metas[batch]['flip'] if 'flip' in img_metas[batch].keys() else False)
                img_crop_offset = (
                    torch.tensor(img_metas[batch]['img_crop_offset']
                ...
                for view_idx in range(5):
                    ...
                    lidar2img_rt = gt_bboxes_3d_center.new_tensor(img_metas[batch]['lidar2img'][view_idx])
                    # breakpoint()
                    # lidar2img_rt = lidar2img_rt * img_scale_factor[0]
                    
                    gt_bboxes_3d = torch.tensor(gt_bboxes_3d)
                    # breakpoint()
                  
                    # gt박스의 코너점들
                    gt_bboxes_2d_corners = []
                    
                    for gt_box_3d in gt_bboxes_3d:
                        transformed_corners = transform_box(gt_box_3d, lidar2img_rt) # transformed_corners.shape = (8X3)
                        
                        transformed_corners[:, 2] = torch.clamp(transformed_corners[:, 2], min=1e-5)
                        transformed_corners[:, 0] /= transformed_corners[:, 2]
                        transformed_corners[:, 1] /= transformed_corners[:, 2]
                        
                        gt_bboxes_2d_corners.append(transformed_corners)
                    # gt_bboxes_2d_corners.shape = (N,8,3)
                    ...
                    gt_bboxes_3d_center_2d = gt_bboxes_3d_center_4d @ lidar2img_rt.t()
                    ...
                    for one_corners in gt_bboxes_2d_corners:
                        one_corners[:, 0:2] = one_corners[:, 0:2] * img_scale_factor
                    ...
                    for one_gt_2d_box_corners in gt_bboxes_2d_corners:
                        one_gt_2d_box_corners = one_gt_2d_box_corners.numpy()
                        img_ = draw_projected_box3d(img_, one_gt_2d_box_corners[:, :2])
                    ...
                    a = cv2.imwrite(save_path + "{}_{}_cam_img_{}_".format(place, scene, view_idx+1) + f'{frame}.png', _img_)
