...
from mmdet3d.datasets import SiT_bev_Dataset
import cv2
from pathlib import Path
import matplotlib.pyplot as plt

sit_categories = ('car', 'truck', 'trailer', 'bus', 'construction_vehicle',
                  'bicycle', 'motorcycle', 'pedestrian', 'traffic_cone',
                  'barrier')

sit_attributes = ('cycle.with_rider', 'cycle.without_rider',
...
def draw_projected_center_point(image, projected_scan, intensity, frame, cam_num, color=(0,255,255), thickness=1):
    projected_scan = np.array(projected_scan)
    
    h,w =image.shape[0:2]
    intensity = intensity[(projected_scan[:,2]>0) * (projected_scan[:,0]>0) * (projected_scan[:,1]>0) * (projected_scan[:,1]<h) * (projected_scan[:,0]<w)]
    projected_scan = projected_scan[(projected_scan[:,2]>0) * (projected_scan[:,0]>0) * (projected_scan[:,1]>0) * (projected_scan[:,1]<h) * (projected_scan[:,0]<w)]
    
    plt.figure(figsize=(19.2/2, 12.0/2))
    plt.imshow(image[:, :, ::-1])
    plt.scatter(projected_scan[:,0], projected_scan[:,1], s=2, alpha=0.7, c=projected_scan[:,2], cmap="rainbow", vmin=0, vmax=40)
    plt.savefig(f"./visual_gt_when_convert/{str(frame)}_{cam_num}.png")
...
def create_sit_infos(root_path,
                          info_prefix,
                          version='sit-trainval',
    ...
    Args:
        ...
        version (str, optional): Version of the data.
            Default: 'sit-trainval'.
        max_sweeps (int, optional): Max number of sweeps.
            Default: 10.
    """
    from nuscenes.utils import splits
    available_vers = ['sit-trainval', 'sit-test']
    assert version in available_vers
    imageset_folder = Path(root_path) / 'ImageSets'
    if version == "sit-trainval":
        if overfit:
            train_img_ids = _read_imageset_file(str(imageset_folder / 'train_overfit.txt'))
            test_img_ids = _read_imageset_file(str(imageset_folder / 'train_overfit.txt'))
            train_scenes = train_img_ids
            val_scenes = train_img_ids
        else:
    ...
    if test:
        ...
        train_sit_infos, val_sit_infos,test_infos = _fill_trainval_infos(
            None, None, None, test_scenes, True, max_sweeps=max_sweeps)        
    else:
        print('train scene: {}, val scene: {}'.format(
            len(train_scenes), len(val_scenes)))
        train_sit_infos, val_sit_infos,test_infos = _fill_trainval_infos(
            None, train_scenes, val_scenes, None, False,max_sweeps=max_sweeps)        

    metadata = dict(version=version)
    if test:
        print('test sample: {}'.format(len(test_infos)))
        ...
        print('train sample: {}, val sample: {}'.format(
            len(train_sit_infos), len(val_sit_infos)))
        if overfit:
            data = dict(infos=train_sit_infos, metadata=metadata)
            info_path = osp.join(root_path,
...
def _fill_trainval_infos(sit,
    ...
    Args:
        sit (:obj:`SiT`): Dataset class in the sit dataset.
    ...
    if test == False:   
        for path in mmcv.track_iter_progress(train_scenes):
            ...
            for num, cam in enumerate(cam_num_list):
                cam_intrinsic=projection_matrix[cam-1][0]
                cam_extrinsic=projection_matrix[cam-1][1]
                cam_extrinsic = np.vstack((cam_extrinsic, [0, 0, 0, 1]))
                cam_extrinsic = np.linalg.inv(cam_extrinsic)

                R = cam_extrinsic[:3, :3]
                T = cam_extrinsic[:3, 3]
                quaternion = Quaternion(matrix=R)
                sensor2ego_rotation = [quaternion.w, quaternion.x, quaternion.y, quaternion.z]
                sensor2ego_translation = T.tolist()
                cam_info = {'data_path':cam_path_[cam-1], 
                            'type': camera_mapping[cam],
            ...
            if not test:
                ...
                gt_velocity = np.zeros((num_objects, 2)) 
        ...
        for path in mmcv.track_iter_progress(test_scenes):
            ...
            sweeps = []
            info['sweeps'] = sweeps
            test_sit_infos.append(info)
                                
    return train_sit_infos, val_sit_infos,test_sit_infos
...
def generate_record_(ann_rec: dict, x1: float, y1: float, x2: float, y2: float,
    ...
    coco_rec['category_id'] = sit_categories.index(cat_name)
    coco_rec['bbox'] = [x1, y1, x2 - x1, y2 - y1]
    coco_rec['iscrowd'] = 0

    return coco_rec

def export_2d_annotation(root_path, info_path, version, mono3d=True, vis_flag=False, pointcloud=False, vis_bev=False):
    ...
    sit_infos = mmcv.load(info_path)['infos']
    cat2Ids = [
        dict(id=sit_categories.index(cat_name), name=cat_name)
        for cat_name in sit_categories
    ]
    coco_ann_id = 0
    coco_2d_dict = dict(annotations=[], images=[], categories=cat2Ids)
    cur_frame = None
    for info in mmcv.track_iter_progress(sit_infos):
        cam_info = info['cams']
        root_path = './data/sit/'
        place = str(cam_info['data_path']).split("/")[2]
        scene = str(cam_info['data_path']).split("/")[3]
        ...
        if vis_bev:
            if frame != cur_frame:
                ...
                dis_points = np.sqrt((points[:,0]**2+points[0,1]**2))
                ax.scatter(points[:,0], points[:,1], s=5, alpha=0.7, c=dis_points,  vmin=0, vmax=40)
                ax.set_xlim(-10,10)
                ax.set_ylim(-10,10)
                cur_frame = frame
                for gt_boxes_corner in gt_boxes_corners:
                    ax = draw_projected_box3d_on_bev(ax, gt_boxes_corner)
                fig.savefig(f"./visual_gt_when_convert_bev_pointcloud/{str(frame)}.png")
...
def get_2d_boxes_(sit,
    ...
    ego_yaw = rotmat_to_euler(ego_motion[:3, :3])[2]
    if vis_flag:
        img = cv2.imread(str(cam_info['data_path']), )
    for i in range(label_['name'].shape[0]):
        ...
        if mono3d and (repro_rec is not None):
            ...
            if repro_rec['center2d'][2] <= 0 or\
                ...
                continue
            if vis_flag:
                img = draw_projected_box3d(img, corners_2d[:, :2])
                img = draw_projected_center(img, center2d)
                
            attr_id = sit_attributes.index(DefaultAttribute[ann_rec['attribute_name']])
            repro_rec['attribute_name'] = DefaultAttribute[ann_rec['attribute_name']]
            repro_rec['attribute_id'] = attr_id
            repro_recs.append(repro_rec)
    if vis_flag:
        if pointcloud:
            img = draw_projected_center_point(img, p_corners_2d[:, :3], intensity, frame, cam_num)
        else:    
            cv2.imwrite(f"./visual_gt_when_convert/{str(frame)}_{cam_num}.png", img)
...
def generate_record(ann_rec: dict, x1: float, y1: float, x2: float, y2: float,
    ...
    if repro_rec['category_name'] not in SiT_bev_Dataset.NameMapping:
        return None
    cat_name = SiT_bev_Dataset.NameMapping[repro_rec['category_name']]
    coco_rec['category_name'] = cat_name
    coco_rec['category_id'] = sit_categories.index(cat_name)
    coco_rec['bbox'] = [x1, y1, x2 - x1, y2 - y1]
    coco_rec['iscrowd'] = 0
