...
def lidar2img(points_lidar, camrera_info):
    ...
    sensor2ego_rotation = Quaternion(camrera_info['sensor2ego_rotation']).rotation_matrix    
    camera2lidar[:3, :3] = sensor2ego_rotation
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default='val',
        help='Version of sit dataset')
    parser.add_argument(
        '--root_path',
        type=str,
        default='./data/sit',
        help='Path to sit dataset')
    parser.add_argument(
        '--save_path',
        type=str,
        default='./vis_bevdepth',
    ...
    parser.add_argument(
        '--fps', type=int, default=1, help='Frame rate of video')
    parser.add_argument(
...
def main():
    ...
    info_path = \
        args.root_path + '/sit_BEV_infos_train_ovf.pkl' 
    ...
    draw_boxes_indexes_img_view = [(0, 1), (1, 2), (2, 3), (3, 0), (4, 5),
                                   ...
                                   (2, 6), (3, 7)]
    views = [
        'CAM_FRONT_LEFT', 'CAM_FRONT', 'CAM_FRONT_RIGHT', 'CAM_BACK_LEFT','CAM_BACK_RIGHT'
    ]
    ...
    for cnt, infos in enumerate(
        ...
        lidar_points = lidar_points.reshape(-1, 4)[:, :3] #error
        ...
        img = np.zeros((900 * 3 + canva_size * scale_factor,1920  * 3, 3),
                       dtype=np.uint8) #(5800, 4800, 3)
        img[:1200, :, :] = np.concatenate(imgs[:3], axis=1) #(1200, 5760, 3)
        img_back = np.concatenate([imgs[3][:, ::-1, :], imgs[4][:, ::-1, :]],axis=1) #(1200, 3840, 3)
        # img[900 + canva_size * scale_factor:, :, :] = img_back
        img[1500 + canva_size * scale_factor:, 1920:, :] = img_back
        img = cv2.resize(img, (int(1600 / scale_factor * 3),
                               int(1200 / scale_factor * 2 + canva_size)))
        w_begin = int((1600 * 3 / scale_factor - canva_size) // 2)
        img[int(1200 / scale_factor):int(1200 / scale_factor) + canva_size,
