...
import pyquaternion
from nuscenes.utils.data_classes import Box as SiTBox
from nuscenes import NuScenes as SiTDaset
...
class SiT_bev_Dataset(Custom3DDataset):
    ...
    CLASSES = ('pedestrian','car',  'bicycle', 'motorcycle', 'bus','truck',)
    ...
    def get(self, table_name: str, token: str) -> dict:
        ...
        if table_name == 'sample':
            return next(item for item in self.data_infos if item['token'] == token)
            ...
            return next(item for item in self.data_infos if item['scene_token'] == token)
        else:
    ...
    def _format_bbox(self, results, jsonfile_prefix=None):
        ...
        for sample_id, det in enumerate(mmcv.track_iter_progress(results)):
            annos = []
            boxes = output_to_sit_box(det, self.with_velocity)
            sample_token = self.data_infos[sample_id]['token']
            boxes = lidar_sit_box_to_global(self.data_infos[sample_id], boxes,
                                             mapped_class_names,
            ...
            for i, box in enumerate(boxes):
                ...
                if np.sqrt(box.velocity[0]**2 + box.velocity[1]**2) > 0.2:
                    attr = SiT_bev_Dataset.DefaultAttribute[name]
                else:
                    attr = SiT_bev_Dataset.DefaultAttribute[name]
                ...
                sit_anno = dict(
                    ...
                    attribute_name=attr)
                annos.append(sit_anno)
            sit_annos[sample_token] = annos
        sit_submissions = {
            'meta': self.modality,
            'results': sit_annos,
        }
        ...
        res_path = osp.join(jsonfile_prefix, 'results_sit.json')
        print('Results writes to', res_path)
        mmcv.dump(sit_submissions, res_path)
        return res_path
    ...
    def _evaluate_single(self,
                         ...
                         show=False):
        """Evaluation for a single model in SiTDaset protocol.
        ...
        eval_set_map = {
            'sit-trainval': 'sit-trainval',
            'sit-test': 'sit-test',
        }
        output_path = Path("./")
        output_path.mkdir(exist_ok=True, parents=True)

        res_path = str(output_path / 'results_pred_sit.json')
        res_gt_path = str(output_path / 'results_gt_sit.json')
        ...
        sit_eval = SiT_Eval(
            ...
            verbose=False)
        sit_eval.main(render_curves=True)
        # record metrics
        metrics = mmcv.load(osp.join(output_dir, 'metrics_summary.json'))
        detail = dict()
        metric_prefix = f'{result_name}_SiTdataset'
    ...
    def format_results(self, results, jsonfile_prefix=None):
        ...
        # this is a workaround to enable evaluation of both formats on SiTDaset
    ...
    def evaluate(self,
                 ...
                 out_dir=None,
                 pipeline=None):
        """Evaluation in SiTDaset protocol.
        ...
        for i, t_ in enumerate(list(result_['results'].keys())):
            ...
            for ii in range(self.data_infos[i]['gt_boxes'].shape[0]):
                ...
                d_['size'] = np.abs(_boxes[3:6][[1,0,2]]).tolist() 
        ...
        save_json(save_path_+'results_pred_sit.json', result_)
        save_json(save_path_+'results_gt_sit.json', gt_json)
...
def output_to_sit_box(detection, with_velocity=False):
    """Convert the output to the box class in the SiTDaset.
    ...
    Returns:
        list[:obj:`sitdatasetBox`]: List of standard sitdatasetBoxes.
    """
    ...
    # box_gravity_center = box3d.tensor[:, :3].clone().detach().numpy()
    box_gravity_center = box3d.center.numpy()
    box_dims = box3d.dims.numpy()
    box_yaw = box3d.yaw.numpy()

    # our LiDAR coordinate system -> SiTDaset box coordinate system lwh -> wlh
...
def lidar_sit_box_to_global(info,
                             boxes,
                             classes,
                             eval_configs,
                             eval_version='detection_cvpr_2019'):
    ...
    Args:
        ...
        boxes (list[:obj:`SiTBox`]): List of predicted SiTBoxes.
    ...
    Returns:
        list: List of standard SiTBoxes in the global
            coordinate.
    """
    # box_list = []
    # for box in boxes:
...
def  add_center_dist_(sit: SiTDaset,
                    eval_boxes: EvalBoxes):
    """
    Adds the cylindrical (xy) center distance from ego vehicle to each box.
    :param sit: The SiTDaset instance.
...
def filter_eval_boxes_(sit: SiTDaset,
                      eval_boxes: EvalBoxes,
                      max_dist: Dict[str, float],
                      verbose: bool = False) -> EvalBoxes:
    """
    Applies filtering to boxes. Distance, bike-racks and points per box.
    :param sit: An instance of the SiTDaset class.
...
class SiT_Eval(DetectionSiTEval):
    def __init__(self,
        ...
        :param sit: A SiTDaset object.
        :param config: A DetectionConfig object.
        :param result_path: Path of the SiTDaset JSON result file.
        ...
        # Load data.
        if verbose:
            print('Initializing SiTDaset detection evaluation')
        ...
        self.pred_boxes = add_center_dist_(sit, self.pred_boxes)
        self.gt_boxes = add_center_dist_(sit, self.gt_boxes)

        # Filter boxes (distance, points per box, etc.).
        if verbose:
            print('Filtering predictions')
        self.pred_boxes = filter_eval_boxes_(sit, self.pred_boxes, self.cfg.class_range, verbose=verbose)
        if verbose:
            print('Filtering ground truth annotations')
        self.gt_boxes = filter_eval_boxes_(sit, self.gt_boxes, self.cfg.class_range, verbose=verbose)
