...
class LoadAnnotations(object):
    ...
    def __call__(self, results):
        ...
        return results
@PIPELINES.register_module()
class Visualizer_gt_point_augmentation(object):
    ...
    def __call__(self, results):
        ...
        lidar_path = results['curr']['lidar_path']
        lidar_points = np.fromfile(lidar_path, dtype=np.float32).reshape(-1, 4)

        for i, sensor in enumerate(sensors):
            img = np.array(results['canvas'][i])
            
            cam_intrinsic = intrins[i].numpy()
            sensor2ego = sensor2egos[i].numpy()
            post_rot = post_rots[i].numpy()
            post_tran = post_trans[i].numpy()
            
            ego2sensor_extrinsic = np.linalg.inv(sensor2ego)

            points_hom = np.hstack((lidar_points[:, :3], np.ones((lidar_points.shape[0], 1))))
            points_sensor = ego2sensor_extrinsic @ points_hom.T

            points_sensor = points_sensor[:3, :] / points_sensor[3, :]

            valid_indices = points_sensor[2, :] > 0
            points_sensor = points_sensor[:, valid_indices]

            points_img = cam_intrinsic @ points_sensor
            points_img = points_img[:2, :] / points_img[2, :]

            points_img = post_rot[:2, :2] @ points_img + post_tran[:2, None]

            for j in range(points_img.shape[1]):
                pt = (int(points_img[0, j]), int(points_img[1, j]))
                cv2.circle(img, pt, 1, (255, 0, 0), -1)

            save_path = os.path.join(self.save_dir, f'{results["sample_idx"]}_img_with_points_{sensor}.png')
