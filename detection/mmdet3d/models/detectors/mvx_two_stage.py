...
class MVXTwoStageDetector(Base3DDetector):
    ...
    def forward_train(self,
        ...
        vis_flag = False
        if vis_flag:
            save_path = "/home/changwon/detection_task/mmdetection3d/viz_in_model/gt/"
            gt_=[]
            for batch in range(len(gt_bboxes_3d)):
                gt_boxes = gt_bboxes_3d[batch].tensor.detach().clone().cpu().numpy()
                point = points[batch].detach().clone().cpu().numpy()
                gt_boxes[:, 6] = -gt_boxes[:, 6]
                #import numpy as np
                #gt_boxes[:, 6] += np.pi/2
                bev = nuscene_vis(point, gt_boxes)
                temp_ = str(img_metas[batch]['pts_filename']).split("/")
                place = temp_[2]
                scene =  temp_[3]
                frame =  temp_[-1].split(".")[0]
                ...
                points_ = points[batch].cpu().detach().numpy()
                np.save("/home/changwon/detection_task/mmdetection3d/viz_in_model/3d_viz/pc/{}.npy".format(img_metas[0]['sample_idx'].split("*")[-1]), points_)
                file = open("/home/changwon/detection_task/mmdetection3d/viz_in_model/3d_viz/box/{}.txt".format(img_metas[0]['sample_idx'].split("*")[-1]), "w")
                with open("/home/changwon/detection_task/mmdetection3d/viz_in_model/3d_viz/box/{}.txt".format(img_metas[0]['sample_idx'].split("*")[-1]), "w") as f:
