...
class SiT_MonoDataset(CocoDataset):
    ...
    def __init__(self,
        ...
        # print(len(self.data_infos)) 27005
    ...
    def get_attr_name(self, attr_idx, label_name):
        ...
        if label_name == 'car' or label_name == 'bus' \
            ...
            if AttrMapping_rev2[attr_idx] == 'vehicle.moving' or \
                ...
                return SiT_MonoDataset.DefaultAttribute[label_name]
    ...
    def _format_bbox(self, results, jsonfile_prefix=None, show=False):
        ...
        if False:
            ...
            for sample_id, det in enumerate(mmcv.track_iter_progress(results)):
                ...
                annos = []
                boxes, attrs = output_to_nusc_box(det)
                sample_token = self.data_infos[sample_id]['token'] #+ "*" +str(sample_id)
                boxes, attrs = cam_nusc_box_to_global_(self.data_infos[sample_id],
                                                    boxes, attrs,
                                                    mapped_class_names,
                                                    self.eval_detection_configs,
    ...
    def _evaluate_single(self,
        ...
        # if show == True:
        #     res_path = str(output_path / 'results_pred_nusc_show.json')
        #     res_gt_path = str(output_path / 'results_gt_nusc_show.json')
    ...
    def evaluate(self,
        ...
        for i, t_ in enumerate(list(result_['results'].keys())):
            ...
            for ii in range(np.array(self.data_infos[data_idx]['gt_boxes']).shape[0]):
                ...
                _token = data_['token']
                # _names = data_['gt_names'][ii]
                if data_['gt_names'][ii]=='pedestrian':
                    _names = 'pedestrian'
                    _boxes = np.array(data_['gt_boxes'])[ii]
                    d_['sample_token'] = _token
                    d_['translation'] = _boxes[:3].tolist() 
        ...
        # else:
            # save_json(save_path_+'results_pred_nusc_show.json', result_)
            # save_json(save_path_+'results_gt_nusc_show.json', gt_json)
    ...
    def show(self, results, out_dir, show=False, pipeline=None, show_bev=False):
        ...
        for i, result in enumerate(results):
            ...
            # threshold = 0.4
            # pred_bboxes = result['boxes_3d'][result['scores_3d'] > threshold]
...
def nusc_box_to_cam_box3d(boxes):
    ...
    # dims = torch.Tensor([b.wlh[[1,2,0]] for b in boxes]).view(-1, 3)
