...
@PIPELINES.register_module()
class ObjectRangeFilterMV(object):
    """Filter objects by the range.
    ...
    def __call__(self, input_dict):
        ...
        for i in range(len(input_dict['gt_bboxes_3d'])):
            ...
            #print('before number of gt boxes:', len(input_dict['gt_bboxes_3d'][i]))

            gt_bboxes_3d = input_dict['gt_bboxes_3d'][i]

            # 문제 정의 
            # 1. gt_bboxes_3d는 각 camera 좌표계 기준으로 나온 값들임
            # 2. range filtering을 적용하기 위해서는 camera -> lidar로의 좌표계 변환이 필요함
            # 3. 이는 extrinsic parameter를 이용해서 lidar 좌표계로 변경을 진행하고,
            # 4. range filtering 용 mask를 구한 후
            # 5. 원래 값에 적용해주면 될듯함
            gt_bboxes_tensor = copy.deepcopy(input_dict['gt_bboxes_3d'][i]).tensor.cpu().numpy()

            gt_bboxes_center = gt_bboxes_tensor[:,:3]

            extrinsic_matrix = np.array(copy.deepcopy(input_dict['cam2img'][i][1]))

            extrinsic_matrix_inv = copy.deepcopy(extrinsic_matrix)
            extrinsic_matrix_inv[:3, :3] = extrinsic_matrix[:3,:3].T
            R_inv = copy.deepcopy(extrinsic_matrix_inv)[:3, :3]
            extrinsic_matrix_inv[:3,3] = -R_inv.dot(extrinsic_matrix[:3,3])

            box_center_3d_homo = np.concatenate((copy.deepcopy(gt_bboxes_center), np.ones_like(gt_bboxes_center[:,:1])),axis=-1) # n * 4

            gt_bboxes_tensor[:,:3] = np.matmul(extrinsic_matrix_inv, box_center_3d_homo.T).T


            
            if isinstance(input_dict['gt_bboxes_3d'][i],
                        (LiDARInstance3DBoxes, DepthInstance3DBoxes)):
                bev_range = self.pcd_range[[0, 1, 3, 4]]
            elif isinstance(input_dict['gt_bboxes_3d'][i], CameraInstance3DBoxes):
                bev_range = self.pcd_range[[0, 2, 3, 5]]
                

            gt_labels_3d = input_dict['gt_labels_3d'][i]

            # append to use in FCOS3D
            gt_centers_2d = input_dict['centers2d'][i]
            gt_depths = input_dict['depths'][i]
            gt_bboxes_2d = input_dict['gt_bboxes'][i]
            gt_labels = input_dict['gt_labels'][i]


            # center 
            object_center = gt_bboxes_tensor[:,:2]

            
            import torch
            mask = torch.tensor((((object_center)[:,0] > bev_range[0])
                            & ((object_center)[:,1] > bev_range[1])
                            & ((object_center)[:,0] < bev_range[2])
                            & ((object_center)[:,1] < bev_range[3])), dtype = torch.bool)
            ...
            # append to use in FCOS3D
            gt_centers_2d = gt_centers_2d[mask.numpy().astype(np.bool)]
            gt_depths = gt_depths[mask.numpy().astype(np.bool)]
            gt_bboxes_2d = gt_bboxes_2d[mask.numpy().astype(np.bool)]
            ...
            # append to use in FCOS3D
            input_dict['gt_bboxes'][i] = gt_bboxes_2d
            input_dict['depths'][i] = gt_depths
            input_dict['centers2d'][i] = gt_centers_2d
            input_dict['gt_labels'][i]= gt_labels

            #print('after number of gt boxes:', len(input_dict['gt_bboxes_3d'][i]))
            #breakpoint()
...
# append from PETR
# https://github.com/megvii-research/PETR/blob/main/projects/mmdet3d_plugin/datasets/pipelines/transform_3d.py
@PIPELINES.register_module()
class PadMultiViewImage(object):
    ...
    def visualize(self, results):
        ...
        for i in range(len(results['img'])):
            # first, draw image
            img = results['img'][i]
            center2d = results['centers2d'][i]
            bbox = results['gt_bboxes'][i]
            bbox3d = results['gt_bboxes_3d'][i]
            projection_matrices = results['cam2img'][i]

            image = Image.fromarray((img).astype('uint8'))
            #image = (img[0, i].permute(1,2,0).detach().cpu().numpy() * 255).astype('uint8')
            image = cv2.cvtColor(np.array(image), cv2.COLOR_BGR2RGB)
        
            img_meta = results['img_info']
            
            # second, draw center of object on image
            center_2d_list = center2d
            
            # align O
            # for j in range(len(center_2d_list)):
            #     x, y = int(center_2d_list[j][0]), int(center_2d_list[j][1])
            #     cv2.circle(image, (x, y), 5, (255,0,0), 5)
            
            # third, draw bbox on image
            # picture is aligned with gt
            bbox_2d_list = bbox
            # for j in range(len(bbox_2d_list)):
            #     xmin, ymin, xmax, ymax = int(bbox_2d_list[j][0]), int(bbox_2d_list[j][1]), int(bbox_2d_list[j][2]), int(bbox_2d_list[j][3])
            #     cv2.rectangle(image, (xmin, ymin), (xmax, ymax), 5, (255,0,0), 5)
            
        
            # 4th, draw 3d bbox on image
            # a) we should conver CameraInstance3D to 8 corners
            # b) should we use projection matrix to draw 3d bbox on image?
            # c) how to draw 3d bbox? 
            #    1) using 2 rectangle box using 8 corners
            #    2) draw rest of line in box
            # from mmdet3d.core.bbox import points_cam2img
            
            bbox_3d_list = bbox3d
            cam2img = projection_matrices[0]
            ...
            cv2.imwrite('./input_img/{}_{}.png'.format(os.path.basename(img_meta['filename'][i]).split('.')[0],
                                                        os.path.basename(os.path.dirname(os.path.dirname(img_meta['filename'][i])))),
                        image)
...
class ResizeMultiview3D:
    ...
    def _resize_img(self, results):
        ...
        for i in range(len(results['img'])):
            ...
            # if not rescale the results are not aligned
            results['cam2img'][i][0][0][0] *= w_scale 
            results['cam2img'][i][0][0][2] *= w_scale
            results['cam2img'][i][0][1][1] *= h_scale
            results['cam2img'][i][0][1][2] *= h_scale

            
            # resize bbox

            if 'gt_bboxes_3d' in results.keys():
                results['gt_bboxes_3d'][i].scale(w_scale) # 잘 변환됨 : yaw를 제외한 나머지 전부가 바뀜


            if 'gt_bboxes' in results.keys():
                for k in range(len(results['gt_bboxes'][i])): # 잘 변환됨
                    results['gt_bboxes'][i][k] *= w_scale
            
                if self.bbox_clip_border:
                    results['gt_bboxes'][i][:, 0::2] = np.clip(results['gt_bboxes'][i][:,0::2], 0, img.shape[1])
                    results['gt_bboxes'][i][:, 1::2] = np.clip(results['gt_bboxes'][i][:,1::2], 0, img.shape[0])
            
            if 'centers2d' in results.keys():
                for l in range(len(results['centers2d'][i])): # 잘 변환됨
                    points_2_cam = points_cam2img(results['gt_bboxes_3d'][i][l].gravity_center, results['cam2img'][i][0], with_depth = True).numpy()[0]
                    results['centers2d'][i][l] = points_2_cam[:2]
    ...
    def visualize(self, results):
        ...
        for i in range(len(results['img'])):
            ...
            # for j in range(len(bbox_2d_list)):
            #     xmin, ymin, xmax, ymax = int(bbox_2d_list[j][0]), int(bbox_2d_list[j][1]), int(bbox_2d_list[j][2]), int(bbox_2d_list[j][3])
            #     cv2.line(image, (xmin, ymin), (xmin, ymax), 5, (255,0,0), 5)
            #     cv2.line(image, (xmin, ymin), (xmax, ymin), 5, (255,0,0), 5)
            #     cv2.line(image, (xmax, ymin), (xmax, ymax), 5, (255,0,0), 5)
            #     cv2.line(image, (xmin, ymax), (xmax, ymax), 5, (255,0,0), 5)
...
# append
@PIPELINES.register_module()
class RandomFlip3DMV(RandomFlipV2):
    """Flip the points & bbox.
    ...
    def __init__(self,
                 ...
                 flip_ratio_bev_vertical=0.0,
                 **kwargs):
        super(RandomFlip3DMV, self).__init__(
            flip_ratio=flip_ratio_bev_horizontal, **kwargs)
        self.sync_2d = sync_2d
        self.flip_ratio_bev_vertical = flip_ratio_bev_vertical
        if flip_ratio_bev_horizontal is not None:
    ...
    def random_flip_data_3d(self, input_dict, direction='horizontal'):
        ...
        """
        assert direction in ['horizontal', 'vertical']
        for i in range(len(input_dict['filename'])):
            ...
            for key in input_dict['bbox3d_fields']:
                if 'points' in input_dict:
                    ...
                    # changed
                    input_dict[key][i].flip(direction)
            if 'centers2d' in input_dict:
                assert self.sync_2d is True and direction == 'horizontal', \
                    'Only support sync_2d=True and horizontal flip with images'
                # (1200, 1920, 3, 4)
                w = input_dict['ori_shape'][1]
                # changed
                input_dict['centers2d'][i][..., 0] = \
                    w - input_dict['centers2d'][i][..., 0]
                # need to modify the horizontal position of camera center
                # along u-axis in the image (flip like centers2d)
                # ['cam2img'][0][2] = c_u
                # see more details and examples at
                ...
                input_dict['cam2img'][i][0][0][2] = w - input_dict['cam2img'][i][0][0][2]
    ...
    def __call__(self, input_dict):
        ...
        super(RandomFlip3DMV, self).__call__(input_dict)
