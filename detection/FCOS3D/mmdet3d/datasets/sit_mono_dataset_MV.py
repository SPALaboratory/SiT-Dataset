...
from ..core.bbox import CameraInstance3DBoxes, get_box_type, LiDARInstance3DBoxes
...
class SiT_MonoDataset_MV(CocoDataset):
    ...
    def __init__(self,
        ...
        if not test_mode:           
            total_valid_inds = self._filter_imgs()

            mask = []
            for j in range(len(self.data_infos)-1, -1, -1):
                if j not in total_valid_inds:
                    mask.append(j)
                    
            for k in mask:
                self.data_infos.pop(k)
                
            proposals = []
            if self.proposals is not None:
                for j, valid_inds in enumerate(total_valid_inds):
                    proposals.append([self.proposals[j][i] for i in valid_inds])
    ...
    def get_ann_info(self, idx):
        ...
        img_token = self.data_infos[idx]
        ann_lists = []
        cnt=0
        for file_name in img_token['file_name']:
            ann_list = []
            for i in range(len(self.coco.anns)):
                new_filename = osp.join('data/sit_full', self.coco.anns[i]['file_name'])
                if new_filename == file_name:
                        ann_list.append(self.coco.anns[i])
            ann_lists.append(ann_list)
            
        return self._parse_ann_info(img_token, ann_lists)
    ...
    def _filter_imgs(self, min_size=32):
            ...
            ids_with_ann = set(_['file_name'] for _ in self.coco.anns.values())
            ids_in_cat = set()
 
            new_img_ids = set(data_info['file_name'][i][14:] for i in range(5) for data_info in self.data_infos)
            
            self.img_ids = new_img_ids
            
            cat_img_map = {0:[], 1:[], 2:[], 3:[], 4:[], 5:[], 6:[], 7:[], 8:[]}
            
            for ann in self.coco.anns.values():
                # only use anno of ped
                if ann['category_id'] in self.cat_ids:
                    cat_img_map[ann['category_id']].append(ann['file_name'])
            
            for i, class_id in enumerate(self.cat_ids):
                ids_in_cat |= set(cat_img_map[class_id])
            ...
            for idx in range(len(self.data_infos)):
                ...
                cnt=0
                for j in range(len(data_info['file_name'])):
                    file_name = data_info['file_name'][j]
                ...
                if cnt!=5:
                    total_valid_inds.append(idx)
            self.img_ids = total_valid_inds
            return total_valid_inds
    ...
    def _parse_ann_info(self, img_infos, ann_infos):
        """Parse bbox annotation.

        Args:
            img_infos (list[dict]): Images info.
            ann_infos (list[dict]): Annotation info of surround images. [num_cam, annotations_per_img]
        ...
        gt_masks_ann_list = []
        gt_bboxes_cam3d_list = []
        centers2d_list = []
        depths_list = []
        gt_labels_3d_list = []
        
    
        for num, ann_info in enumerate(ann_infos):
            '''
            ann_info : 각 image에 대응되는 annotation list
            '''
            gt_bboxes = []
            gt_labels = []
            attr_labels = []
            ...
            # 각 이미지별 대응되는 모든 annotation 정보를 모아서 넘겨줘야 함
            for i, ann in enumerate(ann_info):
                if ann.get('ignore', False):
                    continue
                x1, y1, w, h = ann['bbox']
                inter_w = max(0, min(x1 + w, img_infos['width']) - max(x1, 0))
                inter_h = max(0, min(y1 + h, img_infos['height']) - max(y1, 0))
            ...
            gt_masks_ann_list.append(gt_masks_ann)
            gt_bboxes_cam3d_list.append(gt_bboxes_cam3d)
            centers2d_list.append(centers2d)
            depths_list.append(depths)
        
        
        seg_map_list = []
        for filename in img_infos['file_name']:
            seg_map_list.append(filename.replace('jpg', 'png'))

        ann = dict(
            bboxes=gt_bboxes_list,
            labels=gt_labels_list,
            gt_bboxes_3d=gt_bboxes_cam3d_list,
            gt_labels_3d=copy.deepcopy(gt_labels_list),
            attr_labels=attr_labels_list,
            centers2d=centers2d_list,
            depths=depths_list,
            bboxes_ignore=gt_bboxes_ignore_list,
            masks=gt_masks_ann_list,
    ...
    def get_attr_name(self, attr_idx, label_name):
        ...
        if label_name == 'car' or label_name == 'bus' \
            ...
            if AttrMapping_rev2[attr_idx] == 'vehicle.moving' or \
                ...
                return SiT_MonoDataset_MV.DefaultAttribute[label_name]
    ...
    def _format_bbox(self, results, jsonfile_prefix=None, show=False):
        ...
        for sample_id, det in enumerate(mmcv.track_iter_progress(results)):
            ...
            # currently, sample_token is scene name
            sample_token = self.data_infos[sample_id]['token'] 

            for num, det_dict in enumerate(det[0]):
                # change output box to nusc box in lidar coordinate
                boxes, attrs = output_to_nusc_box_(det_dict['img_bbox'], self.data_infos[sample_id], num)
            ...
            # box nms 3d over 5 images in a frame
        ...
        res_path = osp.join(jsonfile_prefix, 'results_nusc_show.json')
    ...
    def format_results(self, results, jsonfile_prefix=None, show=False, **kwargs):
        ...
        if not ('pts_bbox' in results[0] or 'img_bbox' in results[0]):
            result_files = self._format_bbox(results, jsonfile_prefix, show)
        else:
            ...
            for name in results[0]:
                ...
                result_files.update(
                    {name: self._format_bbox(results_, tmp_file_, show)})
    ...
    def evaluate(self,
        ...
        # append because of filter by range
        def bev(gt_boxes):
            """torch.Tensor: BEV of the points in shape (N, 2)."""
            return np.array(gt_boxes)[:, [0,1]]

        def in_range_bev(gt_boxes, point_range):
            ...
            Returns:
                torch.Tensor: Indicating whether each point is inside
                    the reference range.
            """
            in_range_flags = ((bev(gt_boxes)[:,0] > point_range[0])
                            & (bev(gt_boxes)[:,1] > point_range[1])
                            & (bev(gt_boxes)[:,0] < point_range[2])
                            & (bev(gt_boxes)[:,1] < point_range[3]))
        ...
        result_files, tmp_dir = self.format_results(results, jsonfile_prefix, show)
        save_path_ = "./"
        try:
        ...
        for i, t_ in enumerate(list(result_['results'].keys())):
            gt_json['results'][t_] = []
            for data_idx, cur_data in enumerate(self.data_infos):
                if cur_data['token'] == t_:
                    break
            data_ = self.data_infos[data_idx]


            if len(self.data_infos[data_idx]['gt_boxes'])!=0:
                mask = in_range_bev(copy.deepcopy(self.data_infos[data_idx]['gt_boxes']), [-40., -40., 40., 40.])
                for ii in range(np.array(self.data_infos[data_idx]['gt_boxes']).shape[0]):
                    if ((self.data_infos[data_idx]['gt_names'][ii]=='pedestrian') and (mask[ii])):
                        d_ = {}
                        # currently, _token is scene name
                        _token = data_['token']
                        _names = 'pedestrian'
                        _boxes = np.array(data_['gt_boxes'][ii])
                        d_['sample_token'] = _token
                        d_['translation'] = _boxes[:3].tolist() 
                        d_['size'] = [abs(b) for b in _boxes[3:6].tolist()]
        ...
        if isinstance(result_files, dict):
            ...
            for name in result_names:
                ...
                ret_dict = self._evaluate_single(result_files[name], show=show)
            results_dict.update(ret_dict)
        elif isinstance(result_files, str):
            results_dict = self._evaluate_single(result_files, show=show)

        if tmp_dir is not None:
            tmp_dir.cleanup()
        if show or out_dir:
            self.show(results, out_dir, pipeline=pipeline,show_bev=show_bev)
        return results_dict
    ...
    def show(self, results, out_dir, show=False, pipeline=None, show_bev=False):
        ...
        for i, result in enumerate(results):
            ...
            imgs, f = self._extract_data(i, pipeline,
                                                ['img', 'img_info'])

            for j in range(len(5)):
                img = imgs[i].numpy().transpose(1,2,0)
                pred_bboxes = result['boxes_3d'][j]
                gt_bboxes = self.get_ann_info(i)['gt_bboxes_3d'][j]
                show_multi_modality_result(
                    data_info,
                    img,
                    gt_bboxes,
                    pred_bboxes,
                    torch.tensor(img_metas['cam2img'][j][0]),
                    torch.tensor(img_metas['cam2img'][j][1]),
                    out_dir,
                    file_name,
                    box_mode='camera',
                    show=show,
...
def output_to_nusc_box_(detection, info, num):
    ...
    box_list = []
    if isinstance(detection['boxes_3d'], CameraInstance3DBoxes):
        tensor = copy.deepcopy(detection['boxes_3d']).tensor.cpu().numpy()


        box_center = tensor[:,:3] # n x 3

        cam_extrinsic_parameter = np.array(copy.deepcopy(info['cam_intrinsic'][num][1]))

        rotation_matrix = cam_extrinsic_parameter[:3, :3]
        translation_vector = cam_extrinsic_parameter[:3, 3]

        # inverse of rotation_matrix
        rotation_inverse = rotation_matrix.T

        # inverse of translation_matrix
        translation_inverse = -copy.deepcopy(rotation_inverse).dot(translation_vector)

        # inverse of extrinsic matrix
        transform_inverse = np.identity(4)
        transform_inverse[:3, :3] = rotation_inverse
        transform_inverse[:3, 3] = translation_inverse

        inverse_extrinsic = copy.deepcopy(transform_inverse)[:3,:]

        box_center_3d_homo = np.concatenate((copy.deepcopy(box_center), np.ones_like(box_center[:,:1])),axis=-1) # n * 4

        # ((4 x 4) * (4 x n)).T  = n x 4
        detection['boxes_3d'].tensor[:,:3] = torch.from_numpy(np.matmul(inverse_extrinsic, box_center_3d_homo.T).T[:,:3])
        
        #detection['boxes_3d'].rotate(inverse_extrinsic[:3, :3])
        rot_mat_T = copy.deepcopy(inverse_extrinsic)
        rot_sin = rot_mat_T[2, 0]
        rot_cos = rot_mat_T[0, 0]
        angle = np.arctan2(rot_sin, rot_cos)
        
        detection['boxes_3d'].tensor[:, 6] += angle

        #detection['boxes_3d'].translate(inverse_extrinsic[:3,3])
    ...
    box_gravity_center = box3d.center.numpy()
...
def nusc_box_to_cam_box3d(boxes):
    ...
    # changed because of cuda out of memory
...
class DetectionsitEval(DetectionEval):
    """
    dumy class
    """
...
class SiT_Eval(DetectionsitEval):
    def __init__(self,
                 nusc: NuScenes,
                 config: DetectionConfig,
                 result_path: str,
    ...
    def evaluate(self) -> Tuple[DetectionMetrics, DetectionMetricDataList]:
        ...
        self.cfg.class_names = ['pedestrian']
        dist_ths = self.cfg.dist_ths
        dist_ths = [0.25, 0.5, 1.0, 2.0]
    ...
    def main(self,
        ...
        try:
            with open(os.path.join('.', 'metrics_summary.json'), 'w') as f:
                json.dump(metrics_summary, f, indent=2)
            with open(os.path.join('.', 'metrics_details.json'), 'w') as f:
                json.dump(metric_data_list.serialize(), f, indent=2)
        except:
            pass
