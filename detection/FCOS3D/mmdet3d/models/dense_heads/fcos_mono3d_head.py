...
class FCOSMono3DHead(AnchorFreeMono3DHead):
    ...
    def get_bboxes(self,
        ...
        num_levels = len(cls_scores) # batch size

        # [torch.Size([76, 120]), torch.Size([38, 60]), torch.Size([19, 30]), torch.Size([10, 15]), torch.Size([5, 8])]
        featmap_sizes = [featmap.size()[-2:] for featmap in cls_scores]
        # len(mlvl_points) = batch size
        ...
        # cls_scores : batch size, num_levels, featmap size(H, W)
        result_list = []
        for img_id in range(len(img_metas[0]['filename'])):
            cls_score_list = [
                cls_scores[i][img_id].detach() for i in range(num_levels)
            ]
            bbox_pred_list = [
    ...
    def _get_bboxes_single(self,
        ...
        """
        # if input_meta['filename'][:8] == 'data/sit':
        # chgd
        # try:
        #     view = np.array(input_meta['cam2img'][0]) @ np.array(input_meta['cam2img'][1])
        # except:
        
        # input = input image 1개
        # view = np.array(input_meta['cam2img'][img_id][0])
        view = np.array(input_meta['cam2img'][0])
        scale_factor = input_meta['scale_factor']
        
        #breakpoint()
        # print('before rescale : ', view[0,0], view[0,2], view[1,1], view[1,2])
        if rescale:
            view[0][0] /= scale_factor[0]
            view[0][2] /= scale_factor[0]
            view[1][1] /= scale_factor[1]
            view[1][2] /= scale_factor[1]
        # print('after rescale : ', view[0,0], view[0,2], view[1,1], view[1,2])    
        ...
        for cls_score, bbox_pred, dir_cls_pred, attr_pred, centerness, \
            ...
            if rescale:
                # print('before bbox_pred : ', bbox_pred[0])
                bbox_pred[:, :6] /= bbox_pred[:, :6].new_tensor(scale_factor[0])
                # print('after 1 scale bbox_pred : ', bbox_pred[0])
                bbox_pred[:, 7:] /= bbox_pred[:, 7:].new_tensor(scale_factor[0])
                # print('after 2 scale bbox_pred : ', bbox_pred[0])
        ...
        # bboxes = input_meta['box_type_3d'](bboxes, box_dim=self.bbox_code_size, origin=(0.5, 1.0, 0.5))
    ...
    def get_targets(self, points, gt_bboxes_list, gt_labels_list,
        ...
        #print(len(points)) # [N, GT]
        #print(len(gt_bboxes_list)) # [N, GT, 4]
...
@HEADS.register_module()
class FCOSMono3DHead_MV(AnchorFreeMono3DHead):
    """Anchor-free head used in FCOS3D.
