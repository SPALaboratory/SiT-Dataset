...
def camera_to_lidar_0220(points, P):
    """Convert points in camera coordinate to lidar coordinate.
    ...
    if points.shape[-1] == 3:
        points = np.concatenate([points, np.ones(points_shape + [1])], axis=-1)
    lidar_points = points @ np.linalg.inv(P)
    return lidar_points[..., :3]
...
def box_camera_to_lidar_0220(data, r_rect, velo2cam, P):
    """Convert boxes in camera coordinate to lidar coordinate.
    ...
    x_size, y_size, z_size = data[:, 3:4], data[:, 4:5], data[:, 5:6]
    r = data[:, 6:7]
    xyz_lidar = camera_to_lidar_0220(xyz, r_rect, velo2cam, P)
    # yaw and dims also needs to be converted
    r_new = -r - np.pi / 2
    r_new = limit_period(r_new, period=np.pi * 2)
    return np.concatenate([xyz_lidar, x_size, z_size, y_size, r_new], axis=1)
...
def get_pts_index_in_3dbox__(pc, image_shape):
    num_pts_in_gt = []


    mask_x = np.logical_and(pc[:,0] >= 0, pc[:, 0] <= image_shape[0])
    mask_y = np.logical_and(pc[:,1] >= 0, pc[:, 1] <= image_shape[1])
...
def remove_outside_points_0220(points, rect, Trv2c, P, Tr, image_shape):
    """Remove points which are outside of image.
    ...
    """
    # 5x faster than remove_outside_points_v1(2ms vs 10ms)
    C = np.linalg.inv(P[:3, :3])
    Tr = P[:3, 3].T
    image_bbox = [0, 0, image_shape[1], image_shape[0]]
    frustum = get_frustum(image_bbox, C)
    frustum -= Tr
    frustum = camera_to_lidar_0220(frustum, P)
    frustum_surfaces = corner_to_surfaces_3d_jit(frustum[np.newaxis, ...])
    indices = points_in_convex_polygon_3d_jit(points[:, :3], frustum_surfaces)
    points = points[indices.reshape([-1])]
    return points
...
def get_frustum(bbox_image, C, near_clip=0.001, far_clip=100):
    ...
    # near_box_corners = (box_corners) / np.array(
    #     [1 / near_clip, -1 / near_clip], dtype=C.dtype)
    # far_box_corners = (box_corners) / np.array(
    #     [1 / far_clip, -1 / far_clip], dtype=C.dtype)
