...
def draw_projected_box3d_on_bev(ax, corners_3d_, color='r', thickness=1):
    point_index = [[0,1], [1,5], [5,4],[4,0]]
    for idcs in point_index:
        ax.plot(corners_3d_[idcs][:,0], corners_3d_[idcs][:,1], color)
    center_front = (corners_3d_[4] + corners_3d_[5])/2
    center = corners_3d_[[0,1,4,5]].sum(0)/4
    ax.plot([center_front[0], center[0]], [center_front[1], center[1]], 'y')
    return ax

def show_multi_modality_result(data_info,
                               img,
                               gt_bboxes,
                               pred_bboxes,
                               proj_mat,
    ...
    # if img is not None:
    #     mmcv.imwrite(img, osp.join(result_path, f'{filename}_img_{cam_num}.png'))

    if gt_bboxes is not None:
        gt_img = draw_bbox(
            gt_bboxes, img, proj_mat, img_metas, color=gt_bbox_color)
        mmcv.imwrite(gt_img, osp.join(result_path, f'{filename}_gt_{cam_num}.png'))

    if pred_bboxes is not None:
        pred_img = draw_bbox(
            pred_bboxes, img, proj_mat, img_metas, color=pred_bbox_color)
        mmcv.imwrite(pred_img, osp.join(result_path, f'{filename}_pred_{cam_num}.png'))
    
    
    if show_bev:    
        base_name = osp.basename(out_dir)
        dir_name = osp.dirname(out_dir)
        result_path = osp.join(dir_name, base_name+"_point_cloud", filename)
        mmcv.mkdir_or_exist(result_path)
        
        place = data_info['token'].split('*')[0]
        scene = data_info['token'].split('*')[1]
        frame = data_info['token'].split('*')[2].split(".")[0]
        velo_path = './data/spa/'+ place +'/'+ scene +'/'+ "velo/bin/data/{}.bin".format(frame)
        if gt_bboxes is not None:    
            ...
            gt_bboxes_corners_homo = torch.cat((gt_bboxes_corners, torch.ones_like(gt_bboxes_corners[:,:,0:1])), dim=2) 
            extrinsic_mat_homo = torch.eye(4)
            extrinsic_mat_homo[:3,:] = extrinsic_mat
            gt_bboxes_corners_homo1 = (torch.inverse(extrinsic_mat_homo) @ gt_bboxes_corners_homo.permute(0,2,1)).permute(0,2,1)
            for boxes_corner in gt_bboxes_corners_homo1:
                ax = draw_projected_box3d_on_bev(ax, boxes_corner)
            fig.savefig(osp.join(result_path, f'{filename}_gt_{cam_num}.png'))
        ...
        if pred_bboxes is not None:
            ...
            pred_bboxes_corners = pred_bboxes.corners
            pred_bboxes_corners_homo = torch.cat((pred_bboxes_corners, torch.ones_like(pred_bboxes_corners[:,:,0:1])), dim=2) 
            extrinsic_mat_homo = torch.eye(4)
            extrinsic_mat_homo[:3,:] = extrinsic_mat
            pred_bboxes_corners_homo = (torch.inverse(extrinsic_mat_homo) @ pred_bboxes_corners_homo.permute(0,2,1)).permute(0,2,1)
            for boxes_corner in pred_bboxes_corners_homo:
                ax = draw_projected_box3d_on_bev(ax, boxes_corner, 'b')
            fig.savefig(osp.join(result_path, f'{filename}_pred_{cam_num}.png'))
