...
from mmdet3d.core.bbox import box_np_ops_sit_mvx, points_cam2img
from .sit_mvx_data_utils import WaymoInfoGatherer, get_sit_image_info
from .nuscenes_converter import post_process_coords

sit_mvx_categories = ('pedestrian', 'bicycle', 'car', 'motorcycle')


def convert_to_sit_mvx_info_version2(info):
    """convert sit_mvx info v1 to v2 if possible.

    Args:
        info (dict): Info of the input sit_mvx data.
...
class _NumPointsInGTCalculater:
    ...
    def calculate_single(self, info):
        ...
        if self.remove_outside:
            points_v = box_np_ops_sit_mvx.remove_outside_points(
        ...
        gt_boxes_camera = np.concatenate([loc, dims, rots[..., np.newaxis]],
                                         axis=1)
        gt_boxes_lidar = box_np_ops_sit_mvx.box_camera_to_lidar(
            gt_boxes_camera, rect, Trv2c)
        indices = box_np_ops_sit_mvx.points_in_rbbox(points_v[:, :3], gt_boxes_lidar)
...
def _calculate_num_points_in_gt(data_path,
    ...
    for info in mmcv.track_iter_progress(infos):
        ...
        if remove_outside:
            ...
            pc_mask = box_np_ops_sit_mvx.get_pts_index_in_3dbox__(projected_pc, image_info['image_shape'])
            points_v_ = points_v[pc_mask[0]]

        # points_v = points_v[points_v[:, 0] > 0]
        annos = info['annos']
        num_obj = len([n for n in annos['name'] if n != 'DontCare'])
        # annos = sit_mvx.filter_sit_mvx_anno(annos, ['DontCare'])
        dims = annos['dimensions'][:num_obj]
        loc = annos['location'][:num_obj]
        rots = annos['rotation_y'][:num_obj]
        ...
        gt_boxes_lidar = np.concatenate([loc, dims, rots[..., np.newaxis]],
                                         axis=1)
        #corners_3d = box_np_ops_sit_mvx.box_center_to_corner_3d(gt_boxes_lidar[:, :3], gt_boxes_lidar[:, 3:6], gt_boxes_lidar[:, 6])
        #num_pts_list = box_np_ops_sit_mvx.get_pts_in_3dbox_(points_v, np.expand_dims(corners_3d, axis=0))
        ...
        # gt_boxes_camera = np.concatenate([loc, dims, rots[..., np.newaxis]],
        #                                  axis=1)
        # gt_boxes_lidar = box_np_ops_sit_mvx_mvx.box_camera_to_lidar_0220(
...
def create_sit_mvx_info_file(data_path,
                           pkl_prefix='sit_mvx',
                           with_plane=False,
                           save_path=None,
                           relative_path=True):
    """Create info file of sit_mvx dataset.
    ...
    sit_mvx_infos_train = get_sit_image_info(
        data_path,
        training=True,
        velodyne=True,
        calib=True,
    ...
    _calculate_num_points_in_gt(data_path, sit_mvx_infos_train, relative_path=False)
    filename = save_path / f'{pkl_prefix}_infos_train.pkl'
    print(f'sit_mvx info train file is saved to {filename}')
    mmcv.dump(sit_mvx_infos_train, filename)

    sit_mvx_infos_val = get_sit_image_info(
        data_path,
        training=True,
        velodyne=True,
        calib=True,
        ...
        relative_path=relative_path)
    _calculate_num_points_in_gt(data_path, sit_mvx_infos_val, relative_path)
    filename = save_path / f'{pkl_prefix}_infos_val.pkl'
    print(f'sit_mvx info val file is saved to {filename}')
    mmcv.dump(sit_mvx_infos_val, filename)
    filename = save_path / f'{pkl_prefix}_infos_trainval.pkl'
    print(f'sit_mvx info trainval file is saved to {filename}')
    mmcv.dump(sit_mvx_infos_train + sit_mvx_infos_val, filename)

    sit_mvx_infos_test = get_sit_image_info(
        data_path,
        training=False,
        label_info=False,
        velodyne=True,
    ...
    print(f'sit_mvx info test file is saved to {filename}')
    mmcv.dump(sit_mvx_infos_test, filename)
...
def _create_reduced_point_cloud(data_path,
    ...
    Args:
        ...
        front_camera_id (int, optional): The referenced/front camera ID.
            Default: 2.
    """
    sit_mvx_infos = mmcv.load(info_path)

    for info in mmcv.track_iter_progress(sit_mvx_infos):
        pc_info = info['point_cloud']
        image_info = info['image']
        calib = info['calib']
        ...
        # points_v = box_np_ops_sit_mvx.remove_outside_points(points_v, rect, Trv2c, P, image_info['image_shape'])
        projected_pc = np.dot(P[0], np.dot(P[1][:3, :3], points_v[:, :3].T)+ P[1][:3, 3].reshape(3, 1)).T
        projected_pc[:, 0] /= projected_pc[:, 2]
        projected_pc[:, 1] /= projected_pc[:, 2]
        pc_mask = box_np_ops_sit_mvx.get_pts_index_in_3dbox__(projected_pc, image_info['image_shape'])
...
def export_2d_annotation(root_path, info_path, mono3d=True):
    ...
    sit_mvx_infos = mmcv.load(info_path)
    cat2Ids = [
        dict(id=sit_mvx_categories.index(cat_name), name=cat_name)
        for cat_name in sit_mvx_categories
    ]
    coco_ann_id = 0
...
def get_2d_boxes(info, occluded, mono3d=True):
    ...
    for ann_idx, ann_rec in enumerate(ann_recs):
        ...
        # sensor.
        corners_3d = box_np_ops_sit_mvx.center_to_corner_box3d(
            gt_bbox_3d[:, :3],
            gt_bbox_3d[:, 3:6],
        ...
        for ii, final_coord in enumerate(final_coords):
            ...
            if mono3d and (repro_rec is not None):
                ...
                repro_rec['velo_cam3d'] = -1  # no velocity in sit_mvx
                ...
                repro_rec['attribute_name'] = -1  # no attribute in sit_mvx
...
def generate_record(ann_rec, x1, y1, x2, y2, sample_data_token, filename):
    ...
    if repro_rec['category_name'] not in sit_mvx_categories:
        return None
    cat_name = repro_rec['category_name']
    coco_rec['category_name'] = cat_name
    coco_rec['category_id'] = sit_mvx_categories.index(cat_name)
