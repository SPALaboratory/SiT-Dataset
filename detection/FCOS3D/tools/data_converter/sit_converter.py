...
from mmdet3d.core.bbox import box_np_ops_sit, points_cam2img
from .sit_data_utils import WaymoInfoGatherer, get_sit_image_info
from .nuscenes_converter import post_process_coords

sit_categories = ('Pedestrian', 'Cyclist', 'Car', 'Motorcyclist')


def convert_to_sit_info_version2(info):
    """convert sit info v1 to v2 if possible.

    Args:
        info (dict): Info of the input sit data.
...
class _NumPointsInGTCalculater:
    ...
    def calculate_single(self, info):
        ...
        if self.remove_outside:
            points_v = box_np_ops_sit.remove_outside_points(
        ...
        #                                  axis=1)
        # gt_boxes_lidar = box_np_ops_sit.box_camera_to_lidar(
        #     gt_boxes_camera, rect, Trv2c)
        
        gt_boxes_lidar = np.concatenate([loc, dims, rots[..., np.newaxis]],
                                         axis=1)
                                         
        indices = box_np_ops_sit.points_in_rbbox(points_v[:, :3], gt_boxes_lidar)
...
def _calculate_num_points_in_gt(data_path,
    ...
    for info in mmcv.track_iter_progress(infos):
        ...
        if remove_outside:
            ...
            #     points_v_ = box_np_ops_sit.remove_outside_points_0220(
        ...
        annos = info['annos']
        num_obj = len([n for n in annos['name'] if n != 'DontCare'])
        # annos = sit.filter_sit_anno(annos, ['DontCare'])
        dims = annos['dimensions'][:num_obj]
        loc = annos['location'][:num_obj]
        rots = annos['rotation_y'][:num_obj]
        ...
        # gt_boxes_camera = np.concatenate([loc, dims, rots[..., np.newaxis]],
        #                                  axis=1)
        # gt_boxes_lidar = box_np_ops_sit.box_camera_to_lidar_0220(
...
def create_sit_info_file(data_path,
                           pkl_prefix='sit',
                           with_plane=False,
                           save_path=None,
                           relative_path=True):
    ...
    _calculate_num_points_in_gt(data_path, sit_infos_train, relative_path=False)
    filename = save_path / f'{pkl_prefix}_infos_train.pkl'
    print(f'sit info train file is saved to {filename}')
    mmcv.dump(sit_infos_train, filename)

    sit_infos_val = get_sit_image_info(
        data_path,
        training=True,
        velodyne=True,
        calib=True,
        ...
        relative_path=relative_path)
    _calculate_num_points_in_gt(data_path, sit_infos_val, relative_path)
    filename = save_path / f'{pkl_prefix}_infos_val.pkl'
    print(f'sit info val file is saved to {filename}')
    mmcv.dump(sit_infos_val, filename)
    filename = save_path / f'{pkl_prefix}_infos_trainval.pkl'
    print(f'sit info trainval file is saved to {filename}')
    mmcv.dump(sit_infos_train + sit_infos_val, filename)
    ...
    mmcv.dump(sit_infos_test, filename)
...
def _create_reduced_point_cloud(data_path,
    ...
    for info in mmcv.track_iter_progress(sit_infos):
        ...
        points_v_ = []
        for ii, P in enumerate(P_list):
            # points_v = box_np_ops_sit.remove_outside_points(points_v, rect, Trv2c, P2,
            #                                             image_info['image_shape'])
            points_v__ = box_np_ops_sit.remove_outside_points(points_v, rect, Trv2c, P,
...
def get_2d_boxes(info, occluded, mono3d=True):
    ...
    for ann_idx, ann_rec in enumerate(ann_recs):
        ...
        # sensor.
        # corners_3d = box_np_ops_sit.center_to_corner_box3d(
        #     gt_bbox_3d[:, :3],
        #     gt_bbox_3d[:, 3:6],
        ...
        for ii, final_coord in enumerate(final_coords):
            ...
            if mono3d and (repro_rec is not None):
                ...
                repro_rec['velo_cam3d'] = -1  # no velocity in sit
                ...
                repro_rec['attribute_name'] = -1  # no attribute in sit
...
def generate_record(ann_rec, x1, y1, x2, y2, sample_data_token, filename):
    ...
    if repro_rec['category_name'] not in sit_categories:
        return None
