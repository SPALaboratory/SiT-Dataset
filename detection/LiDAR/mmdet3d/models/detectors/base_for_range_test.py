...
class Base3DDetector(BaseDetector):
    ...
    # def filter_gt(self, points, img_metas, gt_bboxes_3d, gt_labels_3d, img=None, **kwargs):
    ...
    # def forward_test(self, points, img_metas, img=None, **kwargs):
        ...
        """
        # pc_test_range = torch.tensor(self.test_cfg['pts']['point_cloud_range'])  # tensor([-25., -25.,  -5.,  25.,  25.,   3.])
        ...
        if vis_flag:
            ...
            save_path = "./visualize_output_bev_gt_pred/range_512_gt/"
            os.makedirs(save_path, exist_ok=True)
            # breakpoint()

            # gt_bboxes_3d_1=[]
            # for i,box in enumerate(gt_bboxes_3d.tensor.detach().clone().cpu().numpy()):
            #     if (int(pc_test_range[0])<=box[0]<=int(pc_test_range[3])) and (int(pc_test_range[1])<=box[1]<=int(pc_test_range[4])):
            #         gt_bboxes_3d_1.append(box)
            # gt_bboxes_3d_1 = torch.tensor(gt_bboxes_3d_1)
            # breakpoint()
            ...
            for batch in range(len(bbox_list)):
                ...
                # bev_gt = nuscene_vis_gt(point, gt_bboxes_3d[0][batch].tensor.detach().clone().cpu().numpy()) # GT bbox의 BEV map
                bev_gt = nuscene_vis_gt(point, gt_bboxes_3d.tensor.detach().clone().cpu().numpy()) # GT bbox의 BEV map
                cv2.imwrite(save_path+"{}_{}_{}_{}_gt.png".format(rng, place, scene, frame), bev_gt)
                # bev_gt = nuscene_vis_gt(point, gt_bboxes_3d_1.numpy()) # GT bbox의 BEV map
                # breakpoint()
