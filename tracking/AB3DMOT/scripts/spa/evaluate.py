...
class trackingEvaluation(object):
    ...
    def __init__(self, t_sha, gt_path="./scripts/spa", max_truncation = 0, min_height = 25, max_occlusion = 2, \
        ...
        self.t_sha             = t_sha
        self.t_path            = os.path.join("./results/spa", t_sha, "data_%d" % (int(num_hypo)-1))
    ...
    # def compute3rdPartyMetrics(self, threshold=3):
        ...
        for seq_idx in range(len(self.groundtruth)):
            ...
            for track_id, score_list in tracker_id_score.items():
                ...
                #  average_score = round(average_score, 10)
            ...
            #  for f in range(len(seq_tracker)):        # go through each frame
                g = seq_gt[f]
                dc = seq_dc[f]
...
class stat:
    ...
    def plot(self):
        save_dir = os.path.join("./results/spa", self.t_sha)
...
def evaluate(result_sha,mail,num_hypo,eval_3diou,eval_2diou,thres):
    ...
    if eval_3diou:
        mail.msg("Processing Result for spa 3D MOT Benchmark")
    elif eval_2diou:
        mail.msg("Processing Result for spa 2D MOT Benchmark")
    else:
        assert False, 'error'
    classes = []
...
if __name__ == "__main__":
    ...
    if len(sys.argv)!=3 and len(sys.argv)!=4 and len(sys.argv)!=5:
        print("Usage: python3 scripts/spa/evaluate.py result_sha num_hypothesis(e.g., 1) dimension(e.g., 2D or 3D) thres(e.g., 0.25)")
        sys.exit(1);
