...
def _spa_oxts(time_mat, tran_mat, rot_mat):
    poses = [] 
    t_0 = []
    for j in range(len(time_mat)):
        #  if j == 0: continue
        t = np.array(tran_mat[j])
    ...
    if 'spa' in oxts_file:
        ...
        with open(oxts_file, 'r') as f:
            for line in f.readlines():
                ...
                line = [float(i) for i in line]
                time_mat.append([line[0]])
                tran_mat.append(line[2:5])
                rot_mat.append(line[5:])


        imu_poses = _spa_oxts(time_mat, tran_mat, rot_mat)
