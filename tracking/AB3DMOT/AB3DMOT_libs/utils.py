...
def get_subfolder_seq(dataset, split):
    ...
    if dataset == 'KITTI':				# KITTI
        ...
        det_id2str = {3: 'pedestrian'}
        #  det_id2str = {0: 'car', 1: 'bicycle', 2: 'motorcycle', 3: 'pedestrian'}

        subfolder = split
        hw = {'image': (1200, 1920), 'lidar': (720, 1920)}

        #  if split == 'val': seq_eval = ['0000']
        if split == 'val': seq_eval = ['0000', '0001', '0002', '0003', '0004', '0005', '0006']

        data_root = os.path.join(file_path, '../data/spa')
...
def initialize(cfg, data_root, save_dir, subfolder, seq_name, cat, ID_start, hw, log_file):
    ...
    # load ego poses
    oxts = os.path.join(data_root, subfolder, 'oxts', seq_name+'.csv')
