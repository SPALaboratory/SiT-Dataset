...
def load_detection(file):
    ...
    #  ab = np.empty(dets.shape, dtype=decimal.Decimal)
    #  for index, num in np.ndenumerate(dets):
        #  ab[index] = decimal.Decimal(num)
#
    #  dets = ab
...
def save_results(res, save_trk_file, eval_file, det_id2str, frame, score_threshold):
    ...
    bbox3d_tmp, id_tmp, ori_tmp, type_tmp, bbox2d_tmp_trk, conf_tmp = \
        res[0:7], res[7], res[8], det_id2str[int(res[9])], res[10:14], res[14] 		
