...
def parse_args():
    ...
    parser.add_argument("--work_dir", default='./', help="the dir to save logs and tracking results")
    parser.add_argument(
...
def save_first_frame_spa():
    args = parse_args()
    with open(os.path.join('./detection/spa_nusc_infos_val.pkl'), 'rb') as file:
        data = pickle.load(file)

    frames = []
    for sample in nusc:
        for j in range(len(nusc[sample])):
            sample_token = sample.split('*')
            scene_name = sample_token[2]
...
def mk_as_meta(frames):
    output = []
    for i, frm in enumerate(frames):
        cur_dict = {}
        cur_dict['token'] = frm['token']
        token = frm['token']
        timestamp = int(token.split('*')[2])
        cur_dict['timestamp'] = timestamp
        if i % 100 == 0:
            cur_dict['first'] = True
        else:
            cur_dict['first'] = False
...
def main():
    ...
    with open('./detection/results_pred_nusc.json', 'rb') as f:
        predictions=json.load(f)['results']

    with open(os.path.join('./detection/spa_nusc_infos_val.pkl'), 'rb') as file:
        frames = pickle.load(file)['infos']

    frames = mk_as_meta(frames)
