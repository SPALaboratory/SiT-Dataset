...
from utils.preprocessing import augment_data, create_images_dict, augment_rasterize, create_images_val
from utils.image_utils import create_gaussian_heatmap_template, create_dist_mat, \
	preprocess_image_for_segmentation, pad, resize, preprocess_for_torch
from utils.dataloader_for_nsp import SceneDataset, scene_collate
from test_for_nsp import evaluate
...
class YNet:
	...
	def evaluate(self, train_data, val_data, params, train_image_path, val_image_path, experiment_name, batch_size=8, num_goals=20, num_traj=1, device=None, dataset_name=None):
		...
		train_images = create_images_val(train_data, image_path=val_image_path, image_file=image_file_name)
		val_images = create_images_val(val_data, image_path=val_image_path, image_file=image_file_name)
		...
		preprocess_for_torch(train_images, seg_mask=seg_mask)
		...
		val_ADE1, val_FDE1, val_ADE2, val_FDE2, val_ADE3, val_FDE3, val_ADE4, val_FDE4, val_ADE5, val_FDE5, val_ADE6, val_FDE6, val_ADE7, val_FDE7, val_trajFDE7,\
		...
		= evaluate(model, train_loader, train_images, num_goals, num_traj,
		...
		for i in range(3):
			...
			print(f'{verbose[i]}, 7: \nVal ADE: {val_ADE7[i]} \nVal FDE: {val_FDE7[i]}')
			print()
			print(f'{verbose[i]}, 1: \nin_Val ADE: {in_val_ADE1[i]} \nVal FDE: {in_val_FDE1[i]}')
			print(f'{verbose[i]}, 2: \nin_Val ADE: {in_val_ADE2[i]} \nVal FDE: {in_val_FDE2[i]}')
			print(f'{verbose[i]}, 3: \nin_Val ADE: {in_val_ADE3[i]} \nVal FDE: {in_val_FDE3[i]}')
			print(f'{verbose[i]}, 4: \nin_Val ADE: {in_val_ADE4[i]} \nVal FDE: {in_val_FDE4[i]}')
			print(f'{verbose[i]}, 5: \nin_Val ADE: {in_val_ADE5[i]} \nVal FDE: {in_val_FDE5[i]}')
			print(f'{verbose[i]}, 6: \nin_Val ADE: {in_val_ADE6[i]} \nVal FDE: {in_val_FDE6[i]}')
			print(f'{verbose[i]}, 7: \nin_Val ADE: {in_val_ADE7[i]} \nVal FDE: {in_val_FDE7[i]}')
			print()
			print(f'{verbose[i]}, 1: \nout_Val ADE: {out_val_ADE1[i]} \nVal FDE: {out_val_FDE1[i]}')
			print(f'{verbose[i]}, 2: \nout_Val ADE: {out_val_ADE2[i]} \nVal FDE: {out_val_FDE2[i]}')
			print(f'{verbose[i]}, 3: \nout_Val ADE: {out_val_ADE3[i]} \nVal FDE: {out_val_FDE3[i]}')
			print(f'{verbose[i]}, 4: \nout_Val ADE: {out_val_ADE4[i]} \nVal FDE: {out_val_FDE4[i]}')
			print(f'{verbose[i]}, 5: \nout_Val ADE: {out_val_ADE5[i]} \nVal FDE: {out_val_FDE5[i]}')
			print(f'{verbose[i]}, 6: \nout_Val ADE: {out_val_ADE6[i]} \nVal FDE: {out_val_FDE6[i]}')
			print(f'{verbose[i]}, 7: \nout_Val ADE: {out_val_ADE7[i]} \nVal FDE: {out_val_FDE7[i]}')
		print()
		print(f'\nVal FDE_traj: {val_trajFDE7[i]} \nVal FDE: {val_FDE7[i]}')
		print(f'\nVal FDE_traj: {in_val_trajFDE7[i]} \nVal FDE: {in_val_FDE7[i]}')
		print(f'\nVal FDE_traj: {out_val_trajFDE7[i]} \nVal FDE: {out_val_FDE7[i]}')
		# self.val_ADE.append(val_ADE)
		# self.val_FDE.append(val_FDE)

		# if val_ADE < best_test_ADE:
		# 	print(f'Best Epoch {e}: \nVal ADE: {val_ADE} \nVal FDE: {val_FDE}')
		# 	torch.save(model.state_dict(), 'pretrained_models/' + experiment_name + '_weights_test1.pt')
		...
		in_val_ADE1, in_val_FDE1, in_val_ADE2, in_val_FDE2, in_val_ADE3, in_val_FDE3, in_val_ADE4, in_val_FDE4, in_val_ADE5, in_val_FDE5, in_val_ADE6, in_val_FDE6, in_val_ADE7, in_val_FDE7,in_val_trajFDE7, \
		out_val_ADE1, out_val_FDE1, out_val_ADE2, out_val_FDE2, out_val_ADE3, out_val_FDE3, out_val_ADE4, out_val_FDE4, out_val_ADE5, out_val_FDE5, out_val_ADE6, out_val_FDE6, out_val_ADE7, out_val_FDE7, out_val_trajFDE7 \
		= evaluate(model, val_loader, val_images, num_goals, num_traj,
									obs_len=obs_len, batch_size=batch_size,
									device=device, input_template=input_template, params=params,
									waypoints=params['waypoints'], resize=params['resize'],
									temperature=params['temperature'], use_TTST=False,
