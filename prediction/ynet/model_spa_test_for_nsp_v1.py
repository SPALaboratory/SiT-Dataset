...
from test_spa_for_nsp_v1 import evaluate
from train_spa import train
...
class YNet:
    ...
    out_val_ADE1, out_val_FDE1, out_val_ADE2, out_val_FDE2, out_val_ADE3, out_val_FDE3, out_val_ADE4, out_val_FDE4, out_val_ADE5, out_val_FDE5, out_val_trajFDE5 \
		...
		print()
		print(f'\nVal FDE_traj: {val_trajFDE5[i]} \nVal FDE: {val_FDE5[i]}')
		print(f'\nVal FDE_traj: {in_val_trajFDE5[i]} \nVal FDE: {in_val_FDE5[i]}')
		print(f'\nVal FDE_traj: {out_val_trajFDE5[i]} \nVal FDE: {out_val_FDE5[i]}')
	...
	def evaluate(self, data, params, image_path, batch_size=8, num_goals=20, num_traj=1, rounds=1, device=None, dataset_name=None):
		...
		for e in tqdm(range(rounds), desc='Round'):
			...
			test_ADE4, test_FDE4, \
			test_ADE5, test_FDE5, \
       = evaluate(model, test_loader, test_images, num_goals, num_traj,
										  obs_len=obs_len, batch_size=batch_size,
										  device=device, input_template=input_template, params=params,
										  waypoints=params['waypoints'], resize=params['resize'],
