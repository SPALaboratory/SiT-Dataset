...
import json
from utils import raster_util
import matplotlib.pyplot as plt

def augment_rasterize(data, image_path='data/SDD/train', images={}, image_file='reference.jpg', seg_mask=False):
	'''
	Perform data augmentation
	:param data: Pandas df, needs x,y,metaId,sceneId columns
	:param image_path: example - 'data/SDD/val'
	...
	# ks = []
	raster_images = {}
	img_file_name = ['exit2', 'hanyang_plz','hanyang_woman','inter', 'sungsu','sungsu_2','ftc','hangwon','3way']
	img_file_name = ['Courtyard', 'Subway_Entrance', 'Cafeteria', 'Three_way_Intersection', 'Corridor', 'Lobby', 'Outdoor_Alley', 'Crossroad', 'Hallway','Cafe_street']
	print(img_file_name)
	for scene in data.sceneId.unique():
		for location in img_file_name:
			if location in scene:
				img_name = location
				break
		print(image_path)
		im_path = os.path.join(image_path, img_name+'.jpg')#, image_file)
		if seg_mask:
			im = cv2.imread(im_path, 0)
		else:
			im = cv2.imread(im_path)
		# json_file = os.path.join(image_path, scene, image_file)

		# with open(json_file, 'r') as f:
		# 	json_data = json.load(f)
			
		# layer_names = raster_util.check_indoor(json_file)
		# patchbox = (0, 0, 400, 400) #param1
		# angle_in_degrees = 0 #param2
		# canvas_size = (5000, 5000) #param3
		
		# json_data = raster_util.change_keys(json_data)
		# polygons = raster_util.list2Polygon(json_data)
		# im = raster_util.get_map_mask(polygons, patchbox, angle_in_degrees, layer_names, canvas_size)
		# cv2.imwrite(f'./raster/train/{scene}.jpg', im)
		images[scene] = im
		# print(im)
		# print(type(im))
		print(scene)
		raster_images[scene] = im.copy()
	...
	for k in ks:
		metaId_max = data['metaId'].max()
		for scene in data_.sceneId.unique():
			im = raster_images[scene].copy()

			data_rot, im = rot(data_[data_.sceneId == scene], im, k)
			# image
			rot_angle = k2rot[k]
			images[scene + rot_angle] = im
			raster_images[scene + rot_angle] = im.copy()
			data_rot['sceneId'] = scene + rot_angle
			data_rot['metaId'] = data_rot['metaId'] + metaId_max + 1
			data = data.append(data_rot)
	metaId_max = data['metaId'].max()
...
def create_images_val(data, image_path, image_file='reference.jpg'):
	images = {}
	# raster_images = {}
	# for scene in data.sceneId.unique():
	# 	json_file = os.path.join(image_path, scene, image_file)
		
	# 	im_path = os.path.join(image_path, scene+'.jpg')#, image_file)
	# 	im = cv2.imread(im_path)
	# 	with open(json_file, 'r') as f:
	# 		json_data = json.load(f)
			
	# 	layer_names = raster_util.check_indoor(json_file)
	# 	patchbox = (0, 0, 300, 300) #param1
	# 	angle_in_degrees = 0 #param2
	# 	canvas_size = (5000, 5000) #param3
	...
	# 	polygons = raster_util.list2Polygon(json_data)
	# 	im = raster_util.get_map_mask(polygons, patchbox, angle_in_degrees, layer_names, canvas_size)
	# 	cv2.imwrite(f'./raster/val/{scene}.jpg', im)
	# 	images[scene] = im
	# 	raster_images[scene] = im.copy()
	...
	# 	for location in img_file_name:
	# 		if "sungsu" in scene:
	# 			if "sungsu_2_" in scene:
	# 				sungsu = "sungsu2"
	# 			else:
	# 				sungsu = "sungsu1"
	# 		if "sungsu" in location:
	# 			if location == "sungsu_2":
	# 				if sungsu == "sungsu1":
	# 					continue
	# 			elif location == "sungsu":
	# 				if sungsu == "sungsu2":
	# 					continue
	# 		if location in scene:
	# 			img_name = location
	# 			break
		# 	im = cv2.imread(os.path.join(image_path, scene+'.jpg'), 0)
		# except:
		im = cv2.imread(os.path.join(image_path, img_name+'.jpg'))
		images[scene] = im
	return images
