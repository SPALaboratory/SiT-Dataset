...
def evaluate(model, val_loader, val_images, num_goals, num_traj, obs_len, batch_size, device, input_template, waypoints, resize, temperature, params, use_TTST=False, use_CWS=False, rel_thresh=0.002, CWS_params=None, dataset_name=None, homo_mat=None, mode='val', gt_template=torch.ones(1500,1500), no_hd_map=False):
	...
	with torch.no_grad():
		...
		for trajectory, meta, scene, trackID, frame_list, class_id_list, x_list, y_list in tqdm(val_loader):
			...
			for i in range(0, len(trajectory), batch_size):
				...
				for idx, goal_point_ in enumerate([1,5,20]):
					...
					val_FDE1[idx].append(((((gt_future[:, 9:10]  - future_samples[:, :, 9:10]) *4 / 50 / resize) ** 2).sum(dim=3) ** 0.5).min(dim=0)[0])
					val_ADE1[idx].append(((((gt_future[:, :10]   - future_samples[:, :, :10]) *4 / 50 / resize) ** 2).sum(dim=3) ** 0.5).mean(dim=2).min(dim=0)[0])
					val_FDE2[idx].append(((((gt_future[:, 19:20] - future_samples[:, :, 19:20]) *4 / 50 / resize) ** 2).sum(dim=3) ** 0.5).min(dim=0)[0])
					val_ADE2[idx].append(((((gt_future[:, :20]   - future_samples[:, :, :20]) *4 / 50 / resize) ** 2).sum(dim=3) ** 0.5).mean(dim=2).min(dim=0)[0])
					val_FDE3[idx].append(((((gt_future[:, 29:30] - future_samples[:, :, 29:30]) *4 / 50 / resize) ** 2).sum(dim=3) ** 0.5).min(dim=0)[0])
					val_ADE3[idx].append(((((gt_future[:, :30]   - future_samples[:, :, :30]) *4 / 50 / resize) ** 2).sum(dim=3) ** 0.5).mean(dim=2).min(dim=0)[0])
					val_FDE4[idx].append(((((gt_future[:, 39:40] - future_samples[:, :, 39:40]) *4 / 50 / resize) ** 2).sum(dim=3) ** 0.5).min(dim=0)[0])
					val_ADE4[idx].append(((((gt_future[:, :40]   - future_samples[:, :, :40]) *4 / 50 / resize) ** 2).sum(dim=3) ** 0.5).mean(dim=2).min(dim=0)[0])
					val_FDE5[idx].append(((((gt_future[:, 49:50] - future_samples[:, :, 49:50]) *4 / 50 / resize) ** 2).sum(dim=3) ** 0.5).min(dim=0)[0])
					val_ADE5[idx].append(((((gt_future[:, :50]   - future_samples[:, :, :50]) *4 / 50 / resize) ** 2).sum(dim=3) ** 0.5).mean(dim=2).min(dim=0)[0])
					val_FDE6[idx].append(((((gt_future[:, 59:60] - future_samples[:, :, 59:60]) *4 / 50 / resize) ** 2).sum(dim=3) ** 0.5).min(dim=0)[0])
					val_ADE6[idx].append(((((gt_future[:, :60]   - future_samples[:, :, :60]) *4 / 50 / resize) ** 2).sum(dim=3) ** 0.5).mean(dim=2).min(dim=0)[0])
					val_FDE7[idx].append(((((gt_goal - waypoint_samples[:, :, -1:]) *4 / 50 / resize) ** 2).sum(dim=3) ** 0.5).min(dim=0)[0])
					val_ADE7[idx].append(((((gt_future[:, :70]   - future_samples[:, :, :70]) *4 / 50 / resize) ** 2).sum(dim=3) ** 0.5).mean(dim=2).min(dim=0)[0])
					val_trajFDE7[idx].append(((((gt_goal - future_samples[:, :, -1:]) *4 / 50 / resize) ** 2).sum(dim=3) ** 0.5).min(dim=0)[0])
					if ind:
						in_val_FDE1[idx].append(((((gt_future[:, 9:10]  - future_samples[:, :, 9:10]) *4 / 50 / resize) ** 2).sum(dim=3) ** 0.5).min(dim=0)[0])
						in_val_ADE1[idx].append(((((gt_future[:, :10]   - future_samples[:, :, :10]) *4 / 50 / resize) ** 2).sum(dim=3) ** 0.5).mean(dim=2).min(dim=0)[0])
						in_val_FDE2[idx].append(((((gt_future[:, 19:20] - future_samples[:, :, 19:20]) *4 / 50 / resize) ** 2).sum(dim=3) ** 0.5).min(dim=0)[0])
						in_val_ADE2[idx].append(((((gt_future[:, :20]   - future_samples[:, :, :20]) *4 / 50 / resize) ** 2).sum(dim=3) ** 0.5).mean(dim=2).min(dim=0)[0])
						in_val_FDE3[idx].append(((((gt_future[:, 29:30] - future_samples[:, :, 29:30]) *4 / 50 / resize) ** 2).sum(dim=3) ** 0.5).min(dim=0)[0])
						in_val_ADE3[idx].append(((((gt_future[:, :30]   - future_samples[:, :, :30]) *4 / 50 / resize) ** 2).sum(dim=3) ** 0.5).mean(dim=2).min(dim=0)[0])
						in_val_FDE4[idx].append(((((gt_future[:, 39:40] - future_samples[:, :, 39:40]) *4 / 50 / resize) ** 2).sum(dim=3) ** 0.5).min(dim=0)[0])
						in_val_ADE4[idx].append(((((gt_future[:, :40]   - future_samples[:, :, :40]) *4 / 50 / resize) ** 2).sum(dim=3) ** 0.5).mean(dim=2).min(dim=0)[0])
						in_val_FDE5[idx].append(((((gt_future[:, 49:50] - future_samples[:, :, 49:50]) *4 / 50 / resize) ** 2).sum(dim=3) ** 0.5).min(dim=0)[0])
						in_val_ADE5[idx].append(((((gt_future[:, :50]   - future_samples[:, :, :50]) *4 / 50 / resize) ** 2).sum(dim=3) ** 0.5).mean(dim=2).min(dim=0)[0])
						in_val_FDE6[idx].append(((((gt_future[:, 59:60] - future_samples[:, :, 59:60]) *4 / 50 / resize) ** 2).sum(dim=3) ** 0.5).min(dim=0)[0])
						in_val_ADE6[idx].append(((((gt_future[:, :60]   - future_samples[:, :, :60]) *4 / 50 / resize) ** 2).sum(dim=3) ** 0.5).mean(dim=2).min(dim=0)[0])
						in_val_FDE7[idx].append(((((gt_goal - waypoint_samples[:, :, -1:]) *4 / 50 / resize) ** 2).sum(dim=3) ** 0.5).min(dim=0)[0])
						in_val_ADE7[idx].append(((((gt_future[:, :70]   - future_samples[:, :, :70]) *4 / 50 / resize) ** 2).sum(dim=3) ** 0.5).mean(dim=2).min(dim=0)[0])
						in_val_trajFDE7[idx].append(((((gt_goal - future_samples[:, :, -1:]) *4 / 50 / resize) ** 2).sum(dim=3) ** 0.5).min(dim=0)[0])
						
					else:
						out_val_FDE1[idx].append(((((gt_future[:, 9:10]  - future_samples[:, :, 9:10]) *4 / 50 / resize) ** 2).sum(dim=3) ** 0.5).min(dim=0)[0])
						out_val_ADE1[idx].append(((((gt_future[:, :10]   - future_samples[:, :, :10]) *4 / 50 / resize) ** 2).sum(dim=3) ** 0.5).mean(dim=2).min(dim=0)[0])
						out_val_FDE2[idx].append(((((gt_future[:, 19:20] - future_samples[:, :, 19:20]) *4 / 50 / resize) ** 2).sum(dim=3) ** 0.5).min(dim=0)[0])
						out_val_ADE2[idx].append(((((gt_future[:, :20]   - future_samples[:, :, :20]) *4 / 50 / resize) ** 2).sum(dim=3) ** 0.5).mean(dim=2).min(dim=0)[0])
						out_val_FDE3[idx].append(((((gt_future[:, 29:30] - future_samples[:, :, 29:30]) *4 / 50 / resize) ** 2).sum(dim=3) ** 0.5).min(dim=0)[0])
						out_val_ADE3[idx].append(((((gt_future[:, :30]   - future_samples[:, :, :30]) *4 / 50 / resize) ** 2).sum(dim=3) ** 0.5).mean(dim=2).min(dim=0)[0])
						out_val_FDE4[idx].append(((((gt_future[:, 39:40] - future_samples[:, :, 39:40]) *4 / 50 / resize) ** 2).sum(dim=3) ** 0.5).min(dim=0)[0])
						out_val_ADE4[idx].append(((((gt_future[:, :40]   - future_samples[:, :, :40]) *4 / 50 / resize) ** 2).sum(dim=3) ** 0.5).mean(dim=2).min(dim=0)[0])
						out_val_FDE5[idx].append(((((gt_future[:, 49:50] - future_samples[:, :, 49:50]) *4 / 50 / resize) ** 2).sum(dim=3) ** 0.5).min(dim=0)[0])
						out_val_ADE5[idx].append(((((gt_future[:, :50]   - future_samples[:, :, :50]) *4 / 50 / resize) ** 2).sum(dim=3) ** 0.5).mean(dim=2).min(dim=0)[0])
						out_val_FDE6[idx].append(((((gt_future[:, 59:60] - future_samples[:, :, 59:60]) *4 / 50 / resize) ** 2).sum(dim=3) ** 0.5).min(dim=0)[0])
						out_val_ADE6[idx].append(((((gt_future[:, :60]   - future_samples[:, :, :60]) *4 / 50 / resize) ** 2).sum(dim=3) ** 0.5).mean(dim=2).min(dim=0)[0])
						out_val_FDE7[idx].append(((((gt_goal - waypoint_samples[:, :, -1:]) *4 / 50 / resize) ** 2).sum(dim=3) ** 0.5).min(dim=0)[0])
						out_val_ADE7[idx].append(((((gt_future[:, :70]   - future_samples[:, :, :70]) *4 / 50 / resize) ** 2).sum(dim=3) ** 0.5).mean(dim=2).min(dim=0)[0])
						out_val_trajFDE7[idx].append(((((gt_goal - future_samples[:, :, -1:]) *4 / 50 / resize) ** 2).sum(dim=3) ** 0.5).min(dim=0)[0])
		...
		for i in range(3):
			...
			val_ADE6[i] = torch.cat(val_ADE6[i]).mean()
			val_FDE6[i] = torch.cat(val_FDE6[i]).mean()
			...
			in_val_ADE6[i] = torch.cat(in_val_ADE6[i]).mean()
			in_val_FDE6[i] = torch.cat(in_val_FDE6[i]).mean()
			in_val_ADE7[i] = torch.cat(in_val_ADE7[i]).mean()
			in_val_FDE7[i] = torch.cat(in_val_FDE7[i]).mean()
			in_val_trajFDE7[i] = torch.cat(in_val_trajFDE7[i]).mean()
			...
			out_val_ADE6[i] = torch.cat(out_val_ADE6[i]).mean()
			out_val_FDE6[i] = torch.cat(out_val_FDE6[i]).mean()
			out_val_ADE7[i] = torch.cat(out_val_ADE7[i]).mean()
			out_val_FDE7[i] = torch.cat(out_val_FDE7[i]).mean()
			out_val_trajFDE7[i] = torch.cat(out_val_trajFDE7[i]).mean()
   
	return val_ADE1, val_FDE1, val_ADE2, val_FDE2, val_ADE3, val_FDE3, val_ADE4, val_FDE4, val_ADE5, val_FDE5, val_ADE6, val_FDE6, val_ADE7, val_FDE7, val_trajFDE7,\
	in_val_ADE1, in_val_FDE1, in_val_ADE2, in_val_FDE2, in_val_ADE3, in_val_FDE3, in_val_ADE4, in_val_FDE4, in_val_ADE5, in_val_FDE5, in_val_ADE6, in_val_FDE6, in_val_ADE7, in_val_FDE7,in_val_trajFDE7, \
    out_val_ADE1, out_val_FDE1, out_val_ADE2, out_val_FDE2, out_val_ADE3, out_val_FDE3, out_val_ADE4, out_val_FDE4, out_val_ADE5, out_val_FDE5, out_val_ADE6, out_val_FDE6, out_val_ADE7, out_val_FDE7, out_val_trajFDE7
