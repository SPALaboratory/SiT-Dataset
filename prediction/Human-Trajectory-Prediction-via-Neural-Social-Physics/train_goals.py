...
def train(train_batches):
    ...
    for k in shuffle_index[:30]: #Q) 다 안함
        ...
        traj_orig = train_batches[k] #(409, 20, 4)

        batch = 500
        for index in range(0, traj_orig.shape[0], batch):
            if index + batch > traj_orig.shape[0] and index != 0:
                break
            if index + batch > traj_orig.shape[0] and index == 0:
                batch = traj_orig.shape[0]
            
            traj = traj_orig[index:index+batch]
    ...
    return total_loss / batch_num #21JY
...
def test(traj, generated_goals):
    ...
    with torch.no_grad():
        ...
        #     plt.plot(traj[i, :21, 0].cpu(), traj[i, :21, 1].cpu(), 'b-')
        #     plt.plot(traj[i, 21:, 0].cpu(), traj[i, 21:, 1].cpu(), 'r-')
        #     plt.plot(generated_goals[i, 0].cpu(), generated_goals[i, 1].cpu(), 'go')
        #     plt.close()
...
parser.add_argument('--save_file', '-sf', type=str, default='Goals_SiT.pt')
...
# goals_path = 'result_0/result_goal20'
...
try:
    ...
    for i, (test_scene, traj) in enumerate(zip(test_loca_list, test_datas)):
        ...
        predicted_goals = np.swapaxes(np.concatenate(predicted_goals).reshape(-1, 20, 35, 2)[:, :, -1, :], 0, 1) #ICCV
...
for e in range(params['num_epochs']):
    ...
    f=open("./txt/train_goals_ICCV.txt", 'a') #JY
