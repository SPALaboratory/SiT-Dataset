...
def train(train_batches):
    ...
    shuffle_index = torch.randperm(len(train_batches)) 
    for k in shuffle_index[:30]: 
        ...
        for t in range(params['future_length'] - 1):
            ...
            future_vel = (dest - prediction) / ((35-t-1) * 0.2)  # peds*2 #21JY
            future_vel_norm = torch.norm(future_vel, dim=-1)  # peds
            initial_speeds = torch.unsqueeze(future_vel_norm, dim=-1)  # peds*1
...
def test(traj, generated_goals):
    ...
    with torch.no_grad():
        ...
        future_vel = (dest - traj[:, params['past_length'] - 1, :2]) / (torch.tensor(params['future_length']).to(device) * 0.2)  # peds*2 #21JY
        future_vel_norm = torch.norm(future_vel, dim=-1)  # peds
        initial_speeds = torch.unsqueeze(future_vel_norm, dim=-1)  # peds*1
...
parser.add_argument('--save_file', '-sf', type=str, default='SiT_goals.pt')
args = parser.parse_args()

CONFIG_FILE_PATH = 'config/SiT_goals.yaml'  # yaml config file containing all the hyperparameters
...
for e in range(params['num_epochs']):
    ...
    f=open("./txt/train_goals.txt", 'a') #JY
    f.write('\n(epoch {}), ade = {:.3f}, fde = {:.3f}\n'\
