...
def train(path, scenes, isSemanticMap):
    ...
    for i in tqdm(shuffle_index):
        ...
        for index in range(0, traj_complete_translated_total.shape[0], batch):
            ...
            if '_'.join(scene.split('_')[:-1]) == "Crossroad":
                scene_name = "Outdoor_Alley"
            else:
                scene_name = '_'.join(scene.split('_')[:-1])
            ...
            future_vel = (dest - traj[:, params['past_length'] - 1, :2]) / (torch.tensor(params['future_length']).to(device) * 0.1) #peds*2
            ...
            coefficients, curr_supp= model.forward_coefficient_people(outputs_features2, supplement[:, 20, :, :], current_step, current_vel, device)  # peds*maxpeds*2, peds*(max_peds + 1)*4
            ...
            for t in range(params['future_length'] - 1):
                ...
                coefficients, curr_supp = model.forward_coefficient_people(outputs_features2, supplement[:, 20+t+1, :, :], current_step, current_vel, device)
                prediction, w_v = model.forward_next_step(current_step, current_vel, initial_speeds, dest,
    ...
    with torch.no_grad():
        for i, scene in tqdm(enumerate(scenes)):
            ...
            for index in range(0, traj_complete_translated_total.shape[0], batch):
                ...
                predicted_goals = np.swapaxes(np.concatenate(predicted_goals).reshape(-1, 20, 70, 2)[:, :, -1, :], 0, 1)
                for j in range(20):
                    ...
                    for t in range(params['future_length'] - 1):
                        ...
                        coefficients, current_supplement = model.forward_coefficient_people(outputs_features2, supplement[:, 20 + t + 1, :, :],
                    ...
                    test_ade1s = np.mean(np.linalg.norm(y - predictions, axis = 2)[:, :10], axis=1)
                    test_fde1s = np.linalg.norm((y[:,10,:] - predictions[:, 10, :]), axis=1)
                    test_ade3s = np.mean(np.linalg.norm(y - predictions, axis = 2)[:, :30], axis=1)
                    test_fde3s = np.linalg.norm((y[:,30,:] - predictions[:, 30, :]), axis=1)
                    test_ade5s = np.mean(np.linalg.norm(y - predictions, axis = 2)[:, :50], axis=1)
                    test_fde5s = np.linalg.norm((y[:,50,:] - predictions[:, 50, :]), axis=1)
...
parser.add_argument('--save_file', '-sf', type=str, default='SDD_nsp_wo_no_maps_v1_ICCV')
...
load_path = 'saved_models/SDD_goals_no_maps_v1.pt'
...
semantic_path_test = 'data/SDD/val_masks/'
semantic_maps_name_test = os.listdir(semantic_path_test)

goals_path = 'result_no_map_ade1.343788981437683/result_goal20'
goals_list = os.listdir(goals_path)

for e in tqdm(range(params['num_epochs'])):
    total_loss = train(path_train, scenes_train, isSemanticMap=False)
    test_ade, test_fde, ade_1s, fde_1s, ade_3s, fde_3s, ade_5s, fde_5s = test(path_test, scenes_test, goals_path, isSemanticMap=False)

    print()
    f=open("./txt/train_NSP_wo_no_maps_v1_ICCV.txt", 'a') #JY
