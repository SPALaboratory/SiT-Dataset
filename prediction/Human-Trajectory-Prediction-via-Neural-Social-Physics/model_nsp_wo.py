...
class NSP(nn.Module):
    ...
    def __init__(self, input_size, embedding_size, rnn_size, output_size, enc_size, dec_size):
        ...
        self.encoder_robot_state = MLP(input_dim=4, output_dim=output_size, hidden_size=enc_size)
        self.dec_para_people = MLP(input_dim=2 * output_size, output_dim=1, hidden_size=dec_size)
        self.dec_para_robot = MLP(input_dim=1 * output_size, output_dim=1, hidden_size=dec_size)
    ...
    def forward_next_step(self, current_step, current_vel, initial_speeds, dest, features_lstm1, coefficients,
        ...
        delta_t = torch.tensor(0.2).to(device) #21JY
        ...
        tau = self.sigmoid(self.dec_tau(features_tau)) + 0.2 #21JY
