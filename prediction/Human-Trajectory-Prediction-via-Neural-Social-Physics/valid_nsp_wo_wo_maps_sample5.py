    ...
    with torch.no_grad():
        for i, scene in tqdm(enumerate(scenes)):
            ...
            for index in range(0, traj_complete_translated_total.shape[0], batch):
                ...
                ade_20 = np.zeros((5, len(traj_complete[index:index+batch])))
                fde_20 = np.zeros((5, len(traj_complete[index:index+batch])))
                ade_20_1s = np.zeros((5, len(traj_complete[index:index+batch])))
                fde_20_1s = np.zeros((5, len(traj_complete[index:index+batch])))
                ade_20_3s = np.zeros((5, len(traj_complete[index:index+batch])))
                fde_20_3s = np.zeros((5, len(traj_complete[index:index+batch])))
                ade_20_5s = np.zeros((5, len(traj_complete[index:index+batch])))
                fde_20_5s = np.zeros((5, len(traj_complete[index:index+batch])))
                predictions_20 = np.zeros((5, num_peds, params['future_length'], 2))

                with open(os.path.join(generated_goals, scene.split('.')[0] + '.pkl'), 'rb') as f:
                    goals = pickle.load(f)
                predicted_goals = goals
                predicted_goals = np.swapaxes(np.concatenate(predicted_goals).reshape(-1, 5, 35, 2)[:, :, -1, :], 0, 1)
                for j in range(5):
...
parser.add_argument('--save_file', '-sf', type=str, default='SiT_nsp_wo_complete_1.pt')
...
# load_path = 'saved_models/SiT_goals.pt'
...
trained_nsp_wo = torch.load('saved_models/SiT_nsp_wo_complete_1_wo_maps_v1.pt')
model.load_state_dict(trained_nsp_wo['model_state_dict'])
...
goals_path = 'Y-Net_results_noMap/result_goal5'
...
for e in tqdm(range(1)):
    ...
    f=open("./txt/train_NSP_wo_wo_maps_v1_sample5.txt", 'a') #JY
