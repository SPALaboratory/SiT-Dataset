...
def make_prediction_data(made_dir, tracking_info, pad, track_idx,
    ...
    target_list = []
    for idx in np.where(pad.sum(axis=0) == 46)[0].tolist():
...
n_pred = 46 # hist+cur+pred

trajectory_txt = 'trajectory.txt'
src_dir = 'data'
dst_dir = 'deepenai_0603'
local_path = 'preprocess/trajectory_txt'
...
csv_file = pd.read_csv('preprocess/csv/Scene_cutting_.csv')
...
for i in range(len(csv_file)):
    ...
    if selected == 'o':
        train_val_dict[train_val_test].append(Modified_name)
    ...
    for place in places:
        ...
        for cur_scene in loca_files:
            ...
            for seq_idx in range(len(np.unique(frameList))):
                for track_idx in target_ids_self:
                    if_break = False
                    if padding_mask[seq_idx:seq_idx+46, lookup_table[track_idx]].sum() != 46 or tracking_info_np[0, lookup_table[track_idx], 4] != 2.0:
                        continue
                    else:
                        track_num +=1
