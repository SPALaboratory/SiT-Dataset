...
class DataLoader():
    ...
    def __init__(self,f_prefix, batch_size=5, seq_length=20, num_of_validation = 0, forcePreProcess=False, infer=False, generate = False):
        ...
        self.dataset_dimensions = {'hanyang_plz':[720, 576], 'inter':[720, 576], 'sungsu_alley':[595, 326], 'mot':[768, 576]} #Q_JY
        ...
        if infer is False:
            self.base_data_dirs = base_train_dataset #'/data/train/biwi/biwi_hotel.txt'
        else:
            self.base_data_dirs = base_validation_dataset #21JY

        # get all files using python os and base directories #Q_JY_biwi 폴더에 있는 파일들만 가져옴!!!
        ...
        if num_of_validation>0:
            self.additional_validation = True #Q_JY_A_validation 데이터셋중 우리가 원하는 개수만큼 샘플링
        else:
        ...
        if infer is False:
            self.data_dirs = self.train_dataset #['./data/train/biwi/bi...otel_0.txt', './data/train/biwi/bi..._hotel.txt'] #Q_JY
        else:
        ...
        # Number of datasets
        self.numDatasets = len(self.data_dirs) #train 데이터셋 개수

        # array for keepinng target ped ids for each sequence
        self.target_ids = [] #Q_JY

        # Data directory where the pre-processed pickle file resides
        self.train_data_dir = os.path.join(f_prefix, self.base_train_path) #전처리된 데이터 저장 위치 #'./data/train/'
        ...
        self.val_fraction = 0 #Q_JY

        # Define the path in which the process data would be stored
        self.data_file_tr = os.path.join(self.train_data_dir, "trajectories_train.cpkl") #전처리 파일 경로 #'./data/train/trajectories_train.cpkl'
        ...
        self.create_folder_file_dict() #self.folder_file_dict = {'biwi': ['biwi_hotel.txt']}
    ...
    def load_preprocessed(self, data_file, validation_set = False):
        ...
        counter = 0 #Q_JY
        valid_counter = 0 #Q_JY
        print('Sequence size(frame) ------>',46) #21JY
        print('One batch size (frame)--->-', self.batch_size*46) #21JY
        ...
        for dataset in range(len(self.data)):
            ...
            num_seq_in_dataset = int(len(all_frame_data) / (46)) #145 #21JY
            num_valid_seq_in_dataset = int(len(valid_frame_data) / (46)) #21JY
        ...
        self.num_batches = int(counter/self.batch_size) #58
    ...
    def next_batch(self):
        ...
        while i < self.batch_size:
            ...
            if idx + 46-1 < len(frame_data): #21JY
                # All the data in this sequence
                seq_source_frame_data = frame_data[idx:idx+11] #21JY
                seq_numPedsList = numPedsList[idx:idx+11+35] #21JY
                seq_PedsList = pedsList[idx:idx+11+35] #21JY
                seq_target_frame_data = frame_data[idx+11:idx+11+35] #Q_JY

                if len(self.target_ids[self.dataset_pointer][math.floor((self.frame_pointer)/46)]) == 0:
                    self.frame_pointer += 46 #21JY
                    continue
                ...
                # target_ids.append(self.target_ids[self.dataset_pointer][math.floor((self.frame_pointer)/46)]) #21JY
                target_ids.append(self.target_ids[self.dataset_pointer][math.floor((self.frame_pointer)/46)]) #21JY
                self.frame_pointer += 46 #21JY
    ...
    def convert_proper_array(self, x_seq, y_seq, num_pedlist, pedlist):
        ...
        seq_data_x = np.zeros(shape=(11, len(lookup_table), 2)) #(11, 14, 2) #21JY
        seq_data_y = np.zeros(shape=(35, len(lookup_table), 2)) #(35, 14, 2) #21JY
        ...
        return_arr_x = Variable(torch.from_numpy(np.array(seq_data_x)).float()) #torch.Size([20, 3, 2]) #21JY
        return_arr_y = Variable(torch.from_numpy(np.array(seq_data_y)).float()) #torch.Size([20, 3, 2]) #21JY

        return return_arr_x, return_arr_y, lookup_table #21JY
    ...
    def get_dataset_path(self, base_path, f_prefix):
        ...
        dataset = []
        dir_names = unique_list(self.get_all_directory_namelist()) #['biwi']
        for dir_ in dir_names:
            dir_path = os.path.join(f_prefix, base_path, dir_) #'./data/train/biwi'
            file_names = get_all_file_names(dir_path) #['biwi_hotel_0.txt', 'biwi_hotel.txt']
            [dataset.append(os.path.join(dir_path, file_name)) for file_name in file_names]
        return dataset #['./data/train/biwi/biwi_hotel_0.txt', './data/train/biwi/biwi_hotel.txt']
    ...
    def create_folder_file_dict(self):
        ...
        for dir_ in self.base_data_dirs:
            folder_name = dir_.split('/')[-2] #'biwi'
            file_name = dir_.split('/')[-1] #'biwi_hotel.txt'
            self.add_element_to_dict(self.folder_file_dict, folder_name, file_name) #{'biwi': ['biwi_hotel.txt']}
    ...
    def get_all_directory_namelist(self):
        ...
        folder_list = [data_dir.split('/')[-2] for data_dir in (self.base_data_dirs)] #['biwi']
        return folder_list #['biwi']
    ...
    def get_frame_sequence(self, frame_lenght):
        ...
        begin_fr = (self.frame_pointer - 46)
