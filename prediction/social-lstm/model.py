...
class SocialModel(nn.Module):
    ...
    def forward(self, *args):
        ...
        outputs = Variable(torch.zeros((self.seq_length-11) * numNodes, self.output_size)) #21JY
        if self.use_cuda:            
        ...
        if validation_step == False:
            ...
            for framenum,frame in enumerate(target_data):
                ...
                if framenum == (self.seq_length-11)-1:
                    continue
            ...
            nodes_current = self.output_layer(h_nodes)
            outputs[0*numNodes + corr_index.data] = nodes_current
            for framenum in range((self.seq_length-11)-1):
        ...
        # Reshape outputs
        outputs_return = Variable(torch.zeros((self.seq_length-11), numNodes, self.output_size)) #21JY
        if self.use_cuda:
            outputs_return = outputs_return.cuda()
        for framenum in range((self.seq_length-11)): #21JY
            for node in range(numNodes):
