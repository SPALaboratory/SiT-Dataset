...
def main():
    ...
    for batch in range(dataloader.num_batches):
        ...
        for sequence in range(dataloader.batch_size):
            ...
            if sample_args.method == 3: #vanilla lstm
                ret_x_seq, loss = sample_validation_data_vanilla_20(x_seq, y_seq, PedsList_seq, sample_args, net, lookup_seq, numPedsList_seq, dataloader, dataset_data, target_id)

            else:
            	ret_x_seq, loss = sample_validation_data_20(x_seq, y_seq, PedsList_seq, grid_seq, args, net, lookup_seq, numPedsList_seq, dataloader, dataset_data, target_id)

            #revert the points back to original space
            ret_x_seq = revert_seq_20(ret_x_seq, PedsList_seq, lookup_seq, first_values_dict)

            #get mean and final error
            err, err_1s, err_2s, err_3s, err_5s, err_f_1s, err_f_2s, err_f_3s, err_f_5s = \
                get_mean_error_20(ret_x_seq.data, orig_y_seq.data, PedsList_seq[11:], PedsList_seq[11:], args.use_cuda, lookup_seq, target_id)
            f_err = get_final_error_20(ret_x_seq.data, orig_y_seq.data, PedsList_seq[11:], PedsList_seq[11:], lookup_seq, target_id)
    ...
    if dataloader.num_batches != 0:            
        ...
        f=open("./txt/validation_deepenai_social_lstm_sample20.txt", 'a') #JY
