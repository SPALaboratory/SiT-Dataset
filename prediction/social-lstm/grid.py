...
def getGridMask(frame, dimensions, num_person, neighborhood_size, grid_size, is_occupancy = False):
    ...
    if is_occupancy:
        ...
        frame_mask = np.zeros((mnp, mnp, grid_size**2)) #(3, 3, 16)
    frame_np =  frame.data.numpy() #(3, 2)

    #width_bound, height_bound = (neighborhood_size/(width*1.0)), (neighborhood_size/(height*1.0))
    width_bound, height_bound = 10 , 10 #(neighborhood_size/(width*1.0))*2, (neighborhood_size/(height*1.0))*2 #Q_JY_A_타겟으로부터 몇미터까지 볼지 정해주면 됨!!!
    ...
    list_indices = list(range(0, mnp)) #[0, 1, 2]
...
def getSequenceGridMask(sequence, dimensions, pedlist_seq, neighborhood_size, grid_size, using_cuda, is_occupancy=False):
    ...
    for i in range(sl):
        mask = Variable(torch.from_numpy(getGridMask(sequence[i], dimensions, len(pedlist_seq[i]), neighborhood_size, grid_size, is_occupancy)).float()) #torch.Size([3, 3, 16])
        if using_cuda:
            mask = mask.cuda()
