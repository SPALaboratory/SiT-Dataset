...
def get_token(scene_data, sample_data, sample_data_tokens, src_data_token, offset):
    #scene = sample_data[sample_data_tokens.index(src_data_token)]["scene_token"] #src_data_token : palce*scene*frame
        ...
        for idx in np.arange(len(ret_boxes[0])):
            ...
            for t in time:
                ...
                try:
                    ...
                    new_box.center[:2] = new_box.center[:2] + t * velocity
...
def rotation_matrix_to_quaternion(R):
    # Extracting the rotation angles from the rotation matrix
    ...
    qy = (r13 - r31) / (4 * qw)
    qz = (r21 - r12) / (4 * qw)
    # Combining the components into a quaternion array
    q = np.array([qw, qx, qy, qz])
    return q

def _lidar_nusc_box_to_global(nusc, boxes, sample_token):
    sample_token
    place = sample_token.split("*")[0]
    scene = sample_token.split("*")[1]
    frame = sample_token.split("*")[2]
    # ego_path = "./data/spa/{}/{}/ego_trajectory/{}.txt".format(place, scene, frame)
    # ego_matrix = get_ego_matrix(ego_path)
    ...
    for box in boxes:
        ...
        # box.rotate(Quaternion(rotation_matrix_to_quaternion(ego_matrix[:3, :3])))
        # box.translate(np.array(ego_matrix[:3, 3]))
...
def forecast_boxes(nusc, sample_data, scene_data, sample_data_tokens, det_forecast, forecast, forecast_mode, classname, jitter, K, C, train_dist, postprocess):
    ...
    for src, dst in window(ret_tokens, 2):
        elapse_time = get_time_(nusc, src, dst)
...
def _second_det_to_nusc_gt_box(detection):
    box3d = detection["box3d_lidar"]
    scores = detection["scores"]
    mapping_ = {'pedestrian':0, 'car':1}
    labels = np.array([mapping_[i] for i in detection["label_preds"]])
...
def gt_forecast_boxes(nusc, sample_data, scene_data, sample_data_tokens, det_forecast, forecast, forecast_mode, classname, jitter, K, C, train_dist, postprocess):
    ret_boxes, ret_tokens = [], []

    for t in range(forecast):
        dst_token =  get_token(scene_data, sample_data, sample_data_tokens, det_forecast["token"], t)
        ret_tokens.append(dst_token)
    ...
    for t in range(forecast):
        # mask = np.array(det_forecast["name"] == t)
        # box3d = np.concatenate([np.concatenate([det_forecast["location"], det_forecast["dimensions"]], axis=1), det_forecast['rotation_y'].reshape(1,-1)], axis=1)[mask]
        # scores = np.full(det_forecast["name"].shape[0], 1)[mask]
        # name_map={'car':0, 'motorcycle':1, 'pedestrian':2,'bicycle':3, 'motorcycle':4, 'truck':5, 'bus':6, 'kickboard':7}
        # labels = det_forecast["name"][mask] # class mapping

        # box3d = np.concatenate([np.concatenate([det_forecast["location"][:, t, :], det_forecast["dimensions"][:, t, :]], axis=1), det_forecast['rotation_y'][:, t].reshape(-1, 1)], axis=1)

        locs = det_forecast["location"][:, t, :].reshape(-1, 3)
        dims = det_forecast["dimensions"][:, t, :].reshape(-1, 3)
        rots = det_forecast['rotation_y'][:, t].reshape(
                -1, 1
            )
        velos = det_forecast["velocity"][:, t, :].reshape(-1, 2)
        gt_boxes = np.concatenate(
                [locs, dims, velos, rots], axis=1
            )

        scores = np.full(det_forecast["name"].shape[0], 1)
        name_map={'car':0, 'motorcycle':1, 'pedestrian':2,'bicycle':3, 'motorcycle':4, 'truck':5, 'bus':6, 'kickboard':7}
        # labels = det_forecast["name"] # class mapping
        labels = det_forecast["name"][:, t]

        # det = {"box3d_lidar" : box3d, "scores" : scores, "label_preds" : labels, "metadata" : det_forecast["token"]}
        det = {"box3d_lidar" : gt_boxes, "scores" : scores, "label_preds" : labels, "metadata" : det_forecast["token"]}

        boxes = _second_det_to_nusc_gt_box(det)
        #boxes = _lidar_nusc_box_to_global(nusc, boxes, det["metadata"]["token"])
    ...
    if "dense" not in forecast_mode: 
        ...
        # forecast_boxes = tracker(classname, time, ret_boxes)
        forecast_boxes = []
        for idx_ in range(len(ret_boxes[0])): #1~7
            dumy_list = []
            # for t_ in range(7):
            for t_ in range(forecast):
                dumy_list.append(ret_boxes[t_][idx_])
            forecast_boxes.append(dumy_list)
...
class SPA_Nus_Dataset(PointCloudDataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(SPA_Nus_Dataset, self).__init__(
        ...
        # if not hasattr(self, "SPA_Nus_infos"):
        #     self.load_infos(self._info_path)

        self.load_infos(self._info_path)

        self._num_point_features = SPA_Nus_Dataset.NumPointFeatures
    ...
    def ground_truth_annotations(self):
        ...
        for info in self._nusc_infos:
            ...
            #     gt_velocity = info['gt_boxes_velocity'][:,0,:]
            #     gt_track_id = info['gt_track_id'][:, 0]
            # except:
            #     gt_names = np.array(info["gt_names"])
            #     gt_boxes = info["gt_boxes"]
            #     gt_velocity = info['gt_boxes_velocity']
            #     gt_track_id = info['gt_track_id']
            gt_names = np.array(info["gt_names"])
            gt_boxes = info["gt_boxes"]
            gt_velocity = info['gt_boxes_velocity']
            gt_track_id = info['gt_track_id']
            ...
            det_range = np.repeat(det_range[:, np.newaxis, :], repeats=[7], axis=1)
            # det_range = np.repeat(det_range[:, np.newaxis, :], repeats=[3], axis=1)
            # mask = (gt_boxes[:, :2] >= det_range[:, :2]).all(1) # ori
            ...
            #         "velocity": gt_velocity[mask][:, :2],
            #         "gt_track_id": gt_track_id[mask],
            #         "token": info["token"],
            #     }
            gt_annos.append(
                {
                    ...
                    "occluded": np.zeros(N),
                    "truncated": np.zeros(N),
                    "name": gt_names[mask[:, 0]],
                    "location": gt_boxes[mask[:, 0]][:, :, :3],
                    "dimensions": gt_boxes[mask[:, 0]][:, :, 3:6],
                    "rotation_y": gt_boxes[mask[:, 0]][:, :, 6],
                    "velocity": gt_velocity[mask[:, 0]][:, :, :2],
                    "gt_track_id": gt_track_id[mask[:, 0]],
                    "token": info["token"],
                }
            )
        return gt_annos
    ...
    def get_sensor_data(self, idx):
        ...
        vis_flag = False
        if vis_flag:
            save_path = "/home/changwon/detection_task/Det3D/viz_in_model/preprocessing_2/"
            os.makedirs(save_path, exist_ok=True)
            print(self._nusc_infos[idx]['token'])
            print(self._nusc_infos[idx]['gt_boxes_token'][0][0].item())

            # for step in range(7):
            for step in range(forecast):
                gt_boxes = self._nusc_infos[idx]['gt_boxes'][:, step, [0,1,2,3,4,5,-2]]
                token_ = self._nusc_infos[idx]['gt_boxes_token'][0][step]
                ...
                odom_path_ = Path("./data/spa") / place / scene / "ego_trajectory/{}.txt".format(frame)
    ...
    def evaluation(self, detections, output_dir=None, testset=False, forecast=7, forecast_mode="velocity_forward", classname="car", rerank="last", tp_pct=0.6, root="/ssd0/nperi/nuScenes", 
        ...
        # if os.path.isfile(root + "/spa_nusc.pkl"):
        #     nusc = pickle.load(open(root + "/spa_nusc.pkl", "rb"))
        # else:
        #     nusc = NuScenes(version=version, dataroot=root, verbose=True)
        #     pickle.dump(nusc, open(root + "/spa_nusc.pkl", "wb"))
        ...
        sample_data = [s for s in gt_annos]
        sample_data_tokens = [s["token"] for s in gt_annos]
        scene_tokens = [s["token"] for s in gt_annos]
        ...
        for i, token in enumerate(sample_data_tokens):
            #if i % forecast == 0:
            cur_scene_token = token
            scene_data[cur_scene_token] = []
            scene_data[cur_scene_token].append(sample_data_tokens[i])
        ...
        # if True:
            ...
            print(f"Finish generate pred for testset, save to {res_path}")
            ...
            for j, gt_forecast in enumerate(tqdm(gt_annos)):
                gt_boxes, tokens = gt_forecast_boxes(nusc, sample_data, scene_data, sample_data_tokens, gt_forecast, forecast, forecast_mode, classname, jitter, K, C, train_dist, postprocess)
                token = tokens[0]
                annos = []
            ...
            with open(res_path.split("infos_")[0]+"infos_gt_val_01sweeps_withvelo_filter_True.json", "w") as f:
                json.dump(nusc_annos, f)
        
        print(f"Finish generate gt for testset, save to {res_path}")
