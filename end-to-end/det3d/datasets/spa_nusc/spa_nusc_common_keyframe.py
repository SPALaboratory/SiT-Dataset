...
def get_annotations_(nusc, train_scenes, idx, timesteps, root_path):
    ...
    if token == '230412_international*indoor_1_2023-04-12-17-32-46_sync_odom_sync_offset-2_-2_-3_-2_-3_cut1_20sec_5hz*1681288393882005215':
        print(1)
    ...
    for num_ in range(anno['name'].shape[0]): # object별 수행
        ...
        for _i, step in enumerate(range(timesteps)): #0~3
            step *= 5
            if step == 0:
                ...
                try: #train scene의 마지막 idx 이후의 예외처리
                    if len(train_scenes[idx+step-5*(_i-1)]) != 0:
                        if train_scenes[idx+step-5*(_i-1)].split("*")[0] == token.split("*")[0] and train_scenes[idx+step-5*(_i-1)].split("*")[1] == token.split("*")[1]:
                            dumy_anno.update(prev=train_scenes[idx+step-5*(_i-1)], next=train_scenes[idx+step+5*(_i+1)], token=token)
                        else:
                            dumy_anno.update(prev="", next=train_scenes[idx+step+5*(_i+1)], token=token)
                    else:
                        dumy_anno.update(prev="", next=train_scenes[idx+step+5*(_i+1)], token=token)
                    ...
                    # print(1)
                    if len(train_scenes[idx+step-5*(_i-1)]) != 0:
                        dumy_anno.update(prev=train_scenes[idx+step-5*(_i-1)], next=train_scenes[idx], token=token)
                    else:
                ...
                try: #next step이 없을 경우 except로 가서 전스텝을 저장
                    ...
                    if place == place_ and scene == scene_:
                        # token_ = train_scenes[idx+step]
                        # place_ = token_.split("*")[0]
                        # scene_ = token_.split("*")[1]
                        # frame_ = token_.split("*")[2]
                        ...
                        if np.where(anno_['track_id']==track_id)[0].shape[0] != 0: #next step에서 tracked_obj이 있을때만 경로 추가
                            ...
                            try: # next step이 없을 경우 현재 스텝을 저장하도록 함
                                dumy_anno_.update(prev=train_scenes[idx+step-5*(_i-1)], next=train_scenes[idx+step+5*(_i+1)], token=token_)
                            except:
                                dumy_anno_.update(prev=train_scenes[idx+step-5*(_i-1)], next='', token=token_)
        ...
        for ii, _box in enumerate(tracklet_box):
            ...
            try:
                ...
                for_velo = (next_box_center[:3] - cur_box_center[:3]) # 5Hz : 0.2s
...
def _fill_trainval_infos(nusc, train_scenes, val_scenes, test=False, nsweeps=1, filter_zero=True, timesteps=7, past=False):    
    ...
    for ii, path in enumerate(tqdm(train_scenes)):
        if len(train_scenes) - 35 < ii:
        # if len(train_scenes) - 15 < ii:
            continue
    ...
    for ii, path in enumerate(tqdm(val_scenes)):
        if len(val_scenes) - 35 < ii:
        # if len(val_scenes) - 15 < ii:
            continue
