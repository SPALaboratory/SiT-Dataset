    ...
    for num_ in range(anno['name'].shape[0]): # object별 수행
        ...
        for ii, _box in enumerate(tracklet_box):
            ...
            try:
                ...
                for_velo = (next_box_center[:3] - cur_box_center[:3])/1 # 5Hz : 0.2s
...
def _fill_trainval_infos(nusc, train_scenes, val_scenes, test=False, nsweeps=2, filter_zero=True, timesteps=7, past=False):    
    ...
    train_cut_list = _read_imageset_file(root_path / 'ImageSets/test/new_name_e2e_split/train_cut.txt')
    val_cut_list = _read_imageset_file(root_path / 'ImageSets/test/new_name_e2e_split/val_cut.txt')
    ...
    for ii, path in enumerate(tqdm(train_scenes)):
        ...
        if len(train_scenes) - ii <60: #Courtyard*Courtyard_5<-삭제
            continue
        ...
        # if path == "Courtyard*Courtyard_4*120":
        #     pdb.set_trace()
            ...
            if vis_flag:
                for _, bb in enumerate(gt_boxes[mask]):
                    ...
                    for i in range(timesteps):
                        if vis_flag:
                            save_path = "/home/cwkang/data_2/cwkang/SPA_Detection/vis_model/vis_in_model/preprocessing/"
                            os.makedirs(save_path, exist_ok=True)
    ...
    for ii, path in enumerate(tqdm(val_scenes)):
        ...
        if val_scenes[ii] == "Corridor*Corridor_5*63":
            print(1)
...
#                         if train_scenes[idx+step-1].split("*")[0] == token.split("*")[0] and train_scenes[idx+step-1].split("*")[1] == token.split("*")[1]:
...
#                     # comp_obj_center_ = np.matmul(np.linalg.inv(ego_motion_), np.concatenate([anno_['location'][:,:3], np.ones(anno_['location'][:,:3].shape[0]).reshape(1, -1).T], axis=1).T).T
#                     # anno_['location'][:, :3] = comp_obj_center_[:, :3]
#                     # anno_['location'][:, :3] -= ego_motion_[:3, 3] #before
#                     anno_['location'][:, :3] -= ego_motion[:3, 3] # cur step에서의 global box to local #after
...
#                         for key in list(anno_.keys()):
#                             dumy_anno_[key] = np.array(anno_[key][track_index])
#                         # dumy_anno_.update(transform=get_ego_matrix(odom_path_)) #before
#                         dumy_anno_.update(transform=get_ego_matrix(odom_path_)) #after
#                         # dumy_anno_.update(transform=np.eye(4))
#                         try: # next step이 없을 경우 현재 스텝을 저장하도록 함
...
#                         dumy_anno_[key] = np.array(anno_[key][track_index])
#                     # dumy_anno_.update(transform=get_ego_matrix(odom_path_)) #before
#                     dumy_anno_.update(transform=get_ego_matrix(odom_path)) #after
#                     # dumy_anno_.update(transform=np.eye(4))
#                     try: # next step이 없을 경우 현재 스텝을 저장하도록 함
#                         dumy_anno_.update(prev=train_scenes[idx+step-1], next=train_scenes[idx+step+1], token=token_)
#                     except:
...
#     train_cut_list = _read_imageset_file(root_path / 'ImageSets/train_cut.txt')
#     val_cut_list = _read_imageset_file(root_path / 'ImageSets/val_cut.txt')
...
#             # if path == '230412_international*indoor_1_2023-04-12-17-32-46_sync_odom_sync_offset-2_-2_-3_-2_-3_cut6_20sec_5hz*1681288482083865166':
#             #     print(1)
