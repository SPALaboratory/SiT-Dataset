...
def get_label_path_from_token(token):
    ...
    anno_path_ = root_path / place_ / scene_ / "label_3d/{}.txt".format(frame_)
    return [anno_path_]
...
def get_annotations_(nusc, train_scenes, idx, timesteps, root_path):
    ...
    for num_ in range(anno['name'].shape[0]): 
        tracklet_box = []
        tracklet_annotation = []
        tracklet_trajectory = []
        ...
        for step in range(timesteps):
            ...
            if step == 0:
                ...
                # import pdb; pdb.set_trace()
                dumy_anno['global_box'] = np.array(anno_copy['location'][num_])
                dumy_anno.update(transform=get_ego_matrix(odom_path))
                try:
                    if len(train_scenes[idx+step-10]) != 0:
                        if train_scenes[idx+step-10].split("*")[0] == token.split("*")[0] and train_scenes[idx+step-10].split("*")[1] == token.split("*")[1]:
                            dumy_anno.update(prev=train_scenes[idx+step-10], next=train_scenes[idx+step+10], token=token)
                        else:
                            dumy_anno.update(prev="", next=train_scenes[idx+step+10], token=token)
                    else:
                        dumy_anno.update(prev="", next=train_scenes[idx+step+10], token=token)
                    ...
                    # print(1)
                    if len(train_scenes[idx+step-10]) != 0:
                        dumy_anno.update(prev=train_scenes[idx+step-10], next=train_scenes[idx], token=token)
                    else:
                ...
                try:
                    ...
                    if np.where(anno_['track_id']==track_id)[0].shape[0] != 0:
                        ...
                        dumy_anno_['global_box'] = np.array(anno_copy_['location'][track_index])
                        dumy_anno_.update(transform=get_ego_matrix(odom_path_))
                        try: 
                            dumy_anno_.update(prev=train_scenes[idx+step-9], next=train_scenes[idx+step+9], token=token_)
                        except:
                            dumy_anno_.update(prev=train_scenes[idx+step-9], next='', token=token_)
                    ...
                    if np.where(anno_['track_id']==track_id)[0].shape[0] != 0:
                        ...
                        dumy_anno_.update(transform=get_ego_matrix(odom_path_))
                        try: 
                            dumy_anno_.update(prev=train_scenes[idx+step-10], next=train_scenes[idx+step+10], token=token_)
                        except:
                            try:
                                dumy_anno_.update(prev=train_scenes[idx+step-10], next='', token=token_)
                            except:
                                import pdb; pdb.set_trace()
        ...
        for ii, _box in enumerate(tracklet_box):
            ...
            cur_box_global = tracklet_annotation[ii]['global_box']
            ...
            try:
                ...
                next_box_global = tracklet_annotation[ii+1]['global_box']
                
                next_M = tracklet_annotation[ii]["transform"]
                next_box_center = np.matmul(next_M, np.concatenate([next_box_center[:3], np.array([1])]).T).T
                for_velo = (next_box_global[:3] - cur_box_global[:3])/1
                mat_ = np.eye(4)
                mat_[:3, :3] = cur_M[:3, :3]
                for_velo = np.matmul(np.linalg.inv(mat_), np.concatenate([for_velo[:3], np.array([1])]).T).T[:3]
                # import pdb; pdb.set_trace()
                if next_box_global.sum() == 0 or cur_box_global.sum() == 0:
                    import pdb; pdb.set_trace()
                if abs(for_velo[0]) >= 10 or abs(for_velo[1]) >= 10:
                    import pdb; pdb.set_trace()
...
def _fill_trainval_infos(nusc, train_scenes, val_scenes, test=False, nsweeps=2, filter_zero=True, timesteps=7, past=False):   
    train_nusc_infos = []
    val_nusc_infos = []

    root_path = Path("./data/sit/")
    # for path in tqdm(train_scenes):
    train_cut_list = _read_imageset_file(root_path / 'ImageSets/E2E/train_cut.txt')
    val_cut_list = _read_imageset_file(root_path / 'ImageSets/E2E/val_cut.txt')

    for ii, path in enumerate(tqdm(train_scenes)):


        if path in train_cut_list:
            continue
        
        if len(train_scenes) - ii <70: #1s : 10 frame * 7
            continue
        ...
        skip_flag=False
        # nsweeps=10
        while len(sweeps) < nsweeps:
            # if curr_sd_rec["prev"] == "":
            if count== 0 :
                sweep = {
                        "lidar_path": deepcopy(velo_path),
                        "sample_data_token": token,
                        "transform_matrix": ego_motion,
                        "time_lag": frame + "_{}".format(count),
                        # time_lag: 0,
                    }
                sweeps.append(sweep)
                count += 1
        ...
        if not test:
            ...
            mask = np.array([True for pts in num_pts_list], dtype=bool).reshape(-1)
            if mask.sum() == 0:
                import pdb; pdb.set_trace()
                skip_flag_2 = True
            ...
            gt_boxes = [np.concatenate([locs[i], dims[i], velocity[i][:, :2], rvelocity[i][:, :2], rots[i], rrots[i]], axis=1) for i in range(len(forecast_boxes))]
    ...
    for ii, path in enumerate(tqdm(val_scenes)):
        if path in val_cut_list:
            continue
        
        if len(val_scenes) - ii <60:
            continue
        ...
        while len(sweeps) < nsweeps:
            ...
            if count== 0 :
                ...
                next_step_ = val_scenes[ii-count]
                place_ = next_step_.split("*")[0]
                scene_ = next_step_.split("*")[1]
                frame_ = next_step_.split("*")[2]
                token_ = next_step_
...
def get_pts_in_3dbox_(pc, corners):
    ...
    for corner in corners:
        x_max, x_min = np.max(corner[:, 0]), np.min(corner[:, 0])
        y_max, y_min = np.max(corner[:, 1]), np.min(corner[:, 1])
        z_max, z_min = np.max(corner[:, 2]), np.min(corner[:, 2])

        mask_x = (pc[:, 0] >= x_min) & (pc[:, 0] <= x_max)
        mask_y = (pc[:, 1] >= y_min) & (pc[:, 1] <= y_max)
        mask_z = (pc[:, 2] >= z_min) & (pc[:, 2] <= z_max)
        mask = mask_x & mask_y & mask_z
        num_pts_in_box.append(mask.sum())
...
def get_label_anno(label_path):
    ...
    sitting_mask = np.array([True if i != 'pedestrian_sitting' and i != 'car' and i != 'bus' and i != 'bicycle' and i != 'motorcycle' and i != 'truck' else False  for i in annotations['name']])
...
def create_sit_infos(root_path, version="v1.0-trainval", experiment="trainval_forecast", nsweeps=20, filter_zero=True, timesteps=7, past=False):
    nusc = NuScenes(version=version, dataroot=root_path, verbose=True)
    available_vers = ["v1.0-trainval", "v1.0-test", "v1.0-mini", "v1.0-sit-trainval"]
    ...
    if version == "v1.0-trainval":
        ...
        imageset_folder = Path(root_path) / 'ImageSets/E2E/'
