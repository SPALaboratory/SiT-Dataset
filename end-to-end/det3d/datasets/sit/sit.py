...
def _lidar_nusc_box_to_global(nusc, boxes, sample_token):
    ...
    ego_path = "./data/sit/{}/{}/ego_trajectory/{}.txt".format(place, scene, frame)
    ego_matrix = get_ego_matrix(ego_path)
    ego_yaw = rotmat_to_euler(ego_matrix[:3, :3])[2]
    # import pdb; pdb.set_trace()

    box_list = []
    for box in boxes:
        cur_box_center = box.center
        cur_box_center = np.matmul(ego_matrix, np.concatenate([cur_box_center[:3], np.array([1])]).T).T
        box.center = cur_box_center[:3]
        box.rotate(Quaternion(axis=[0,0,1], radians=-ego_yaw))
...
def _second_det_to_nusc_gt_box(detection):
    ...
    mapping_ = {'pedestrian':0, 'car':1, 'bus':2}
...
class SiT_Dataset(PointCloudDataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(SiT_Dataset, self).__init__(
        ...
        self._num_point_features = SiT_Dataset.NumPointFeatures
    ...
    def evaluation(self, detections, output_dir=None, testset=False, forecast=7, forecast_mode="velocity_forward", classname="car", rerank="last", tp_pct=0.6, root="/ssd0/nperi/nuScenes", 
        ...
        # if True:
            ...
            with open(res_path.split("infos_")[0]+"infos_gt_val_10sweeps_withvelo_filter_True.json", "w") as f:
                json.dump(nusc_annos, f)
